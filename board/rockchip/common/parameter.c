...
int get_rdInfo(PBootInfo pboot_info)
{
	...
	while (token != NULL) {
		if (!strncmp(token, "initrd=", len)) {
			...
			pboot_info->ramdisk_load_addr =
			    simple_strtoull(token, &token, 0);
			pboot_info->ramdisk_size = 0x80000;

			if (*token == ',') {
				token++;
				pboot_info->ramdisk_size =
				    simple_strtoull(token, NULL, 0);
			}
...
int get_part(char *parts, mtd_partition * this_part, int *part_index)
{
	...
	if (*parts == '-') {	/* assign all remaining space to this partition */
		size = SIZE_REMAINING;
		parts++;
	} else {
...
int mtdpart_parse(const char *string, cmdline_mtd_partition * this_mtd)
{
	...
	if (s != NULL) {
		...
		s = p + 1;
		get_part(s, this_mtd->parts + this_mtd->num_parts,
			 &(this_mtd->num_parts));
	}
...
//              }
		...
		pboot_info->index_misc =
		    find_mtd_part(&pboot_info->cmd_mtd, PARTNAME_MISC);
		pboot_info->index_kernel =
		    find_mtd_part(&pboot_info->cmd_mtd, PARTNAME_KERNEL);
		pboot_info->index_boot =
		    find_mtd_part(&pboot_info->cmd_mtd, PARTNAME_BOOT);
		pboot_info->index_recovery =
		    find_mtd_part(&pboot_info->cmd_mtd, PARTNAME_RECOVERY);
		pboot_info->index_system =
		    find_mtd_part(&pboot_info->cmd_mtd, PARTNAME_SYSTEM);
		pboot_info->index_backup =
		    find_mtd_part(&pboot_info->cmd_mtd, PARTNAME_BACKUP);
		pboot_info->index_snapshot =
		    find_mtd_part(&pboot_info->cmd_mtd, PARTNAME_SNAPSHOT);

		if (pboot_info->index_backup > 0) {
			g_FwEndLba =
			    pboot_info->cmd_mtd.parts[pboot_info->
						      index_backup].
			    offset +
			    pboot_info->cmd_mtd.parts[pboot_info->
						      index_backup].size;
...
int setup_key(char *line, key_config * key)
{
	...
	if (keyTemp.type == KEY_GPIO) {
		...
		if (*(line++) != ',')
			return -1;
		keyTemp.key.gpio.index =
		    (sub_group - 'A') * 8 + ParseAtoi(line++);
		if (*(line++) != ',')
			return -1;
		...
		*key = keyTemp;
		...
		RkPrintf("%d,%d,%d,%d\n", key->type, key->key.gpio.group,
			 key->key.gpio.index, key->key.gpio.valid);

		RkPrintf("%08X,%08X,%08X,%08X\n", key->key.gpio.io_read,
			 key->key.gpio.io_write, key->key.gpio.io_dir_conf,
			 key->key.gpio.io_debounce);
#endif
	} else if (keyTemp.type == KEY_AD) {	// 其它类型的按键定义
		if (*(line++) != ',')
			return -1;
		...
		if (*(line++) != ',')
			if (*(line++) != ',')
				return -1;
		if (keyTemp.key.adc.keyValueLow >=
		    keyTemp.key.adc.keyValueHigh)
			return -1;
		*key = keyTemp;
...
void ParseLine(PBootInfo pboot_info, char *line)
{
	if (!memcmp(line, "MAGIC:", strlen("MAGIC:")))
		pboot_info->magic_code =
		    ParseAtoi(line + strlen("MAGIC:"));
		...
		pboot_info->machine_type =
		    ParseAtoi(line + strlen("MACHINE:"));
	else if (!memcmp(line, "CHECK_MASK:", strlen("CHECK_MASK:")))
		pboot_info->check_mask =
		    ParseAtoi(line + strlen("CHECK_MASK:"));
	else if (!memcmp(line, "KERNEL_IMG:", strlen("KERNEL_IMG:")))
		krnl_load_addr = pboot_info->kernel_load_addr =
		    ParseAtoi(line + strlen("KERNEL_IMG:"));
	else if (!memcmp(line, "BOOT_IMG:", strlen("BOOT_IMG:")))
		pboot_info->boot_offset =
		    ParseAtoi(line + strlen("BOOT_IMG:"));
	else if (!memcmp(line, "RECOVERY_IMG:", strlen("RECOVERY_IMG:")))
		pboot_info->recovery_offset =
		    ParseAtoi(line + strlen("RECOVERY_IMG:"));
	else if (!memcmp(line, "MISC_IMG:", strlen("MISC_IMG:")))
		pboot_info->misc_offset =
		    ParseAtoi(line + strlen("MISC_IMG:"));
#if 0
	else if (!memcmp(line, "RECOVER_KEY:", strlen("RECOVER_KEY:"))) {	//  RECOVER_KEY: 0,4,A,2,0  ==> GPIO4 PA2 低有效
	...
	} else if (!memcmp(line, "PWR_HLD:", strlen("PWR_HLD:"))) {	//  RECOVER_KEY: 0,4,A,2,0  ==> GPIO4 PA2 低有效
		line += strlen("PWR_HLD:");
		EATCHAR(line, ' ');
	    ...
	    if (!memcmp
		(line, "COMBINATION_KEY:", strlen("COMBINATION_KEY:"))) {
...
//      PRINT_D("Enter\n");
	...
	//if( pParam->tag != 0xFFFFFFFF && pParam->tag != 0 )
	{
		...
		crc =
		    CRC_32CheckBuffer((unsigned char *)pParam->parameter,
				      pParam->length + 4);
		if (!crc) {
...
//              RkPrintf("%s\n", line);
		// 去除空行及注释行
		if (line[0] != 0 && line[0] != '#') {	// 该行不是空行，并且不是注释行
