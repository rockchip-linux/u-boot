...
void EnumDone(void) 
{
	...
	usbCmd.Precmd = 0;
	    ...
	    (1ul << 28) | (1 << 15) | (2 << 18) | (BULK_IN_EP << 22);
...
void ControlHandler(void) 
{
	    ...
	    ControlData.DeviceRequest.bmRequestType & USB_REQUEST_TYPE_MASK;
...
void OutIntr(void) 
{
	...
	for (i = 0; i < 3; i++)
			    ...
			    OtgReg->Device.OutEp[i].DoEpInt & OtgReg->Device.
			    doepmsk;
			if ((event & 0x01) != 0)	//Transfer complete
			{
					    ...
					    0x20000 -
					    (OtgReg->Device.OutEp[BULK_OUT_EP].
					     DoEpTSiz & 0x1ffff);
