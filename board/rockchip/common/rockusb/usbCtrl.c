...
void UsbHook(void) 
{
			    ...
			    if (0)	//(usbCmd.xferLBA == usbCmd.preLBA)&&(usbCmd.preLen >= curLen))
			{
				    ...
				    (curLen >
				     usbCmd.preLen) ? usbCmd.preLen : curLen;
			    ...
			    if (0)	//usbCmd.len >= 0x80)
			{
				...
				StorageReadPba(usbCmd.preLBA, usbCmd.preBuf,
						curLen);
				...
				&& (usbCmd.preLen >= curLen))
					...
					UsbStorageSysDataLoad(usbCmd.xferLBA -
							       0xFFFFFF00,
							       curLen,
							       usbCmd.xferBuf);
			    ...
			    if (usbCmd.len >= 0x20)
				    ...
				    //transfer complete
				    StorageReadLba(usbCmd.preLBA, usbCmd.preBuf,
						   curLen);
			...
			if (SecureBootLock == 0)
				...
				StorageWriteLba(curLBA, pBuf, (curLen >> 9),
						 FW_IMG_WR_Mode);
