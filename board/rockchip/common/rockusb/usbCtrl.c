...
int dwc_otg_check_dpdm(void)
{
	...
	otg_hprt0 =
	    (unsigned int *)(OtgReg + DWC_OTG_HOST_PORT_REGS_OFFSET);
	...
	while (1) {
		if (usbCmd.cmd == K_FW_READ_10) {
			...
			if (0)	//(usbCmd.xferLBA == usbCmd.preLBA)&&(usbCmd.preLen >= curLen))
			{
				...
				StorageReadPba(usbCmd.xferLBA,
					       usbCmd.xferBuf, curLen);
			}
			...
			if (0)	//usbCmd.len >= 0x80)
			{
				...
				usbCmd.preBuf =
				    bulkBuf[usbCmd.pppBufId & 0x01];
				usbCmd.preLBA = preLBA;
				usbCmd.preLen = curLen;
				StorageReadPba(usbCmd.preLBA,
					       usbCmd.preBuf, curLen);
			}
			...
			//use the pre-read data first
			if ((usbCmd.xferLBA == usbCmd.preLBA)
			    && (usbCmd.preLen >= curLen)) {
				usbCmd.xferBuf = usbCmd.preBuf;
				curBuf = usbCmd.pppBufId;
				curLen =
			...
			OTPFlashRead(usbCmd.xferLBA, usbCmd.xferBuf,
				     curLen);
#else
			SPIFlashRead(usbCmd.xferLBA, usbCmd.xferBuf,
				     curLen);
#endif
			preLBA = usbCmd.xferLBA;
			xferLen = usbCmd.xferLen;
			...
			if (usbCmd.preLen == usbCmd.len) {
				...
				len =
				    ((usbCmd.len - usbCmd.preLen) >
				     0x40) ? 0x40 : (usbCmd.len -
						     usbCmd.preLen);
				usbCmd.xferBuf =
				    bulkBuf[usbCmd.pppBufId & 0x01];
				usbCmd.xferLBA += (curLen / 528);
				ReadBulkEndpoint(len * 528,
						 usbCmd.xferBuf);
			}
			...
			if (g_BootRockusb != 2) {
				if (((GetVbus() == 0)
					...
					g_BootRockusb = 0;
					//powerOn();
					change_cmd_for_recovery(&gBootInfo,
								recv_cmd);
					RkPrintf(" %d %d\n", TimeOutBase,
						 get_ticks());
