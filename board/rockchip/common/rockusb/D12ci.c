...
void WriteEndpoint0(uint16 len, void *buf) 
{
	...
	 if (((OtgReg->Core.gnptxsts & 0xffff) >= (len + 3) / 4)
	       && (((OtgReg->Core.gnptxsts >> 16) & 0xff) > 0))
...
void ReadBulkEndpoint(uint32 len, void *buf) 
{
	    ...
	    OtgReg->Device.OutEp[BULK_OUT_EP].DoEpTSiz =
	    0x20000 | (((len + BulkEpSize - 1) / BulkEpSize) << 19);
...
//    if ((OtgReg->Device.InEp[BULK_IN_EP].DTXFSTS & 0xffff) >= (len+3)/4)
	{
		OtgReg->Device.InEp[BULK_IN_EP].DiEpTSiz =
		    len | (((len + BulkEpSize - 1) / BulkEpSize) << 19);
		OtgReg->Device.InEp[BULK_IN_EP].DiEpDma = (uint32) buf;
		regBak =
		    ((OtgReg->Device.InEp[BULK_IN_EP].DiEpCtl & (1 << 16)) ==
		     0) ? (1 << 28) : (1 << 29);
