...
extern void FW_CheckMultiBadBlock(uint8 ChipSel, uint32 blkNum,
				  uint32 BlockCnt, uint64 * pResult);
...
{
	return ((input >> 24) | (input << 24) | (input >> 8 & 0xff00) |
		(input << 8 & 0xff0000));
}
...
void FWOutPacket(uint32 len)
{
	...
	if (FWCmdPhase == K_CommandPhase) {
		...
		if (gCBW.Code == K_FW_SDRAM_EXECUTE) {
			...
			FW_SDRAM_Parameter =
			    ((uint32) pchar[25]) | ((uint32) pchar[24] <<
						    8) | ((uint32)
							  pchar[23] << 16)
			    | ((uint32) pchar[22] << 24);
		}
	...
	//接收命令
		...
		if (FW_WR_Mode == FW_WR_MODE_PBA) {
...
#if 0
				CacheFlushDRegion((uint32) DataBuf,
						  (uint32) gCBW.Len * 528);
				FW_FlashWritePBA(gCBW.LUN, gCBW.LBA,
						 DataBuf, gCBW.Len);
				SendCSW();
#else
			...
			if ((uint32) gCBW.Len <= FW_DataLenCnt) {
				uint8 *pSdram =
				    (uint8 *) (SDRAM_BASE_ADDRESS +
					       gCBW.LBA);
				CacheFlushDRegion((uint32) DataBuf,
						  (uint32) gCBW.Len);
				ftl_memcpy(pSdram, (uint8 *) DataBuf,
					   FW_DataLenCnt);
				memset(pSdram + FW_DataLenCnt, 0, 512);
				SendCSW();
				FW_DataLenCnt = 0;
			} else {
				ReadBulkEndpoint(len,
						 &DataBuf[FW_DataLenCnt /
							  4]);
			}
		}
			...
			if ((uint32) gCBW.Len <= FW_DataLenCnt) {
				...
				CacheFlushDRegion((uint32) DataBuf,
						  (uint32) gCBW.Len);
				SPIFlashWrite(gCBW.LBA, DataBuf,
					      FW_DataLenCnt);
				SendCSW();
#else
...
void FW_Read10(void)
{
	...
	byte =
	    (dCSWDataResidueVal >
	     BulkEpSize) ? BulkEpSize : dCSWDataResidueVal;
	FW_DataLenCnt = 0;
	WriteBulkEndpoint(byte, &DataBuf[FW_DataLenCnt]);
	dCSWDataResidueVal -= byte;
	FW_DataLenCnt += byte;
	FWCmdPhase =
	    (dCSWDataResidueVal > 0) ? K_InDataPhase : K_InCSWPhase;
#else
	CSWHandler(CSW_GOOD, gCBW.XferLen);
...
void FW_GetChipVer(void)
{
	...
	Rk30ChipVerInfo[0] = 0;
	ftl_memcpy(Rk30ChipVerInfo, (uint8 *) (BOOT_ROM_CHIP_VER_ADDR),
		   16);
