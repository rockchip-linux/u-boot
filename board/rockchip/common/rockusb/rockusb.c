...
void FWOutPacket(uint32 len) 
{
	    ...
	    if (FWCmdPhase == K_CommandPhase)
		    ...
		    CacheFlushDRegion((uint32) & gCBW, sizeof(gCBW));
			    ...
			    ((uint32) pchar[25]) | ((uint32) pchar[24] << 8) |
			    ((uint32) pchar[23] << 16) | ((uint32) pchar[22] <<
							  24);
	    ...
	    //接收命令
				    ...
				    CacheFlushDRegion((uint32) DataBuf,
						      (uint32) gCBW.Len * 528);
				FW_FlashWritePBA(gCBW.LUN, gCBW.LBA, DataBuf,
						  gCBW.Len);
			...
			if (1)	//  (((uint32)gCBW.Len*512 <= FW_DataLenCnt) || (FW_DataLenCnt==0x10000))
			{
				    ...
				    (uint8 *) (SDRAM_BASE_ADDRESS + gCBW.LBA);
...
void FW_Read10(void) 
{
	    ...
	    (dCSWDataResidueVal > BulkEpSize) ? BulkEpSize : dCSWDataResidueVal;
