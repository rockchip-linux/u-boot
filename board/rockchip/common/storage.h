...
extern bool FW_FlashPageProg(uint8 ChipSel, uint32 row, void *pData,
			     void *pSpare, uint8 nSec);
extern bool FW_CheckBadBlock(uint8 ChipSel, uint32 pageNum);
extern void FW_SetBadBlock(uint8 ChipSel, uint32 pageNum);
extern uint8 FW_FlashReadPage(uint8 ChipSel, uint32 row, void *pData,
			      void *pSpare, uint8 nSec);
extern void FlashReadID(uint8 ChipSel, void *buf);
extern void ReadFlashIDInfo(uint8 * buf);
extern uint32 FlashBootErase(uint8 ChipSel, uint32 blkIndex, uint32 nblk,
			     uint8 mod);
extern void FW_ReIntForUpdate(void);
extern void FW_SorageLowFormatEn(int en);

extern int StorageWriteLba(uint32 LBA, void *pbuf, uint16 nSec,
			   uint16 mode);
...
extern uint32 UsbStorageSysDataLoad(uint32 offset, uint32 len,
				    uint32 * Buf);
extern uint32 UsbStorageSysDataStore(uint32 offset, uint32 len,
				     uint32 * Buf);

//local memory operation function
typedef uint32(*Memory_Init) (uint32 BaseAddr);
typedef uint32(*Memory_ReadPba) (uint8 ChipSel, uint32 PBA, void *pbuf,
				 uint16 nSec);
typedef uint32(*Memory_WritePba) (uint8 ChipSel, uint32 PBA, void *pbuf,
				  uint16 nSec);
typedef uint32(*Memory_ReadLba) (uint8 ChipSel, uint32 LBA, void *pbuf,
				 uint16 nSec);
typedef uint32(*Memory_WriteLba) (uint8 ChipSel, uint32 LBA, void *pbuf,
				  uint16 nSec, uint16 mode);
typedef uint32(*Memory_Erase) (uint8 ChipSel, uint32 blkIndex, uint32 nblk,
			       uint8 mod);
...
typedef uint32(*Memory_SysDataLoad) (uint8 ChipSel, uint32 Index,
				     void *Buf);
typedef uint32(*Memory_SysDataStore) (uint8 ChipSel, uint32 Index,
				      void *Buf);
...
typedef uint32(*Memory_FlashReadLba) (uint8 ChipSel, uint32 LBA,
				      uint16 nSec, void *pbuf);
typedef uint32(*Memory_FlashWriteLba) (uint8 ChipSel, uint32 LBA,
				       uint16 nSec, void *pbuf);
