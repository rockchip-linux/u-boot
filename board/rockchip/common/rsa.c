...
unsigned int digits;
{
	...
	if (c != 0) {
		for (i = 0; i < digits; i++) {
			...
			if ((a[i] =
			     b[i] - borrow) > (MAX_NN_DIGIT - borrow))
				borrow = 1;
			else
				borrow = 0;
			 ...
			 R_RSA_PUBLIC_KEY * publicKey)
{
	...
	NN_Decode(n, MAX_NN_DIGITS, publicKey->modulus,
		  MAX_RSA_MODULUS_LEN);
	NN_Decode(e, MAX_NN_DIGITS, publicKey->exponent,
		  MAX_RSA_MODULUS_LEN);
