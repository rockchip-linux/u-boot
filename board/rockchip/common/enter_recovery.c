...
void change_cmd_for_recovery(PBootInfo boot_info, char *rec_cmd)
{
	if (boot_info->index_recovery >= 0) {
		...
		sprintf(szFind, "%s=%s:", "mtdparts",
			boot_info->cmd_mtd.mtd_id);
		...
		if (s != NULL) {
			...
			char replace_str[64] = "";
			//parameter is 4M.
			sprintf(replace_str, "0x00002000@0x%08X(%s),", 0,
				"parameter");
			s += strlen(szFind);
	...
	} else;			//PRINT_E("Try to boot recovery system, but can't find recovery partition!\n");
}
#endif
