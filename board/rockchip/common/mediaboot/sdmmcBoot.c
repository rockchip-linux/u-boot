...
uint32	sdmmc_SpareBuf[(32*8*4/4)]; __attribute__((aligned(ARCH_DMA_MINALIGN)));
uint32	sdmmc_Data[(1024*8*4/4)]; __attribute__((aligned(ARCH_DMA_MINALIGN)));
...
uint32 SdmmcBootWritePBA(uint8 ChipSel, uint32 PBA , void *pbuf, uint32 nSec )
{
    ...
    {
        //memset(sdmmc_SpareBuf,0xFF,SPARE_LEN);
    ...
    for (len=0; len<nSec; len+=pageSizeLimit)
    {
        for (i=0; i<(MIN(nSec,pageSizeLimit)); i++)
        {
            ...
            //ftl_memcpy(sdmmc_SpareBuf+i*4, pDataBuf+(len+i)*132+128, 16);
        }
...
uint32 SdmmcBootReadPBA(uint8 ChipSel, uint32 PBA , void *pbuf, uint32 nSec )
{
    ...
    for (len=0; len<nSec; len+=pageSizeLimit)
    {
        ...
        for (i=0; i<(MIN(nSec,pageSizeLimit)); i++)
        {
            ...
            //ftl_memcpy(pDataBuf+(len+i)*132+128, sdmmc_SpareBuf+(i)*4, 16);
        }
    }
...
void EmmcIdBlockRecover(uint8 part)
{
    ...
    BootCapSize = gSdCardInfoTbl[EMMC_CARD_ID].BootCapSize;
    if(gEmmcBootPart != part && BootCapSize > 0) 
    {  
        uint32 ioctlParam[5] = {0,0,0,0,0};
        IDBlockData0 = (uint32*)0x61000000;//&usbXferBuf[0];
        IDBlockData1 = (uint32*)0x61100000;//&usbXferBuf[IDblockDataSize/4];
        ioctlParam[0] = EMMC_CARD_ID;
        ioctlParam[1] = gEmmcBootPart;
        gSdCardInfoTbl[EMMC_CARD_ID].AccessPart = gEmmcBootPart;
        SDM_IOCtrl(SDM_IOCTR_ACCESS_BOOT_PARTITION, ioctlParam);
        if(SDM_Read(EMMC_CARD_ID,0,BootCapSize,IDBlockData1) != SDM_SUCCESS)
        {
            return ;
        }
        ...
        SDM_IOCtrl(SDM_IOCTR_ACCESS_BOOT_PARTITION, ioctlParam);
        SDM_Read(EMMC_CARD_ID,0,BootCapSize,IDBlockData0);
        if(ftl_memcmp(IDBlockData1,IDBlockData0,BootCapSize<<9) == 0)
        {
        ...
        if(SDM_Write(EMMC_CARD_ID,0,BootCapSize,IDBlockData1) != SDM_SUCCESS)
        {
            return;
        }
        //check data
...
int sdBootCheckSdCard(uint8 ChipSel)
{
    ...
    if(gSdCardInfoTbl[ChipSel].FwPartOffset == SD_CARD_FW_PART_OFFSET)
    {
        ...
        //rknand_print_hex("mbr", buf, 2, 256);
        if(buf[255] == 0xAA55 && buf[0] == 0x0 && buf[4] == 0x0 && buf[8] == 0x0 && buf[64] == 0x0 && buf[128]==0) //检查表的合法性
        {
