...
extern int do_booti(cmd_tbl_t * cmdtp, int flag, int argc,
		    char *const argv[]);
extern void change_cmd_for_recovery(PBootInfo boot_info, char *rec_cmd);
extern int checkKey(uint32 * boot_rockusb, uint32 * boot_recovery,
...
static int do_format(void)
{
	...
	for (n = 0, next = 0; fbt_partitions[n].name; n++) {
		...
		if (add_ptn
		    (ptbl, next, next + sz - 1, fbt_partitions[n].name))
			return -1;
		next += sz;
	}
	end_ptbl(ptbl);
	...
	blocks_to_write =
	    DIV_ROUND_UP(sizeof(struct ptable), dev_desc->blksz);
	result =
	    dev_desc->block_write(dev_desc->dev, 0, blocks_to_write, ptbl);
...
int board_fbt_key_pressed(void)
{
	...
	int vbus = GetVbus();
	checkKey((uint32 *) & boot_rockusb, (uint32 *) & boot_recovery,
		 (uint32 *) & boot_fastboot);
	printf("vbus = %d\n", vbus);
...
void board_fbt_finalize_bootargs(char *args, int buf_sz,
				 int ramdisk_addr, int ramdisk_sz,
				 int recovery)
{
	char recv_cmd[2] = { 0 };
