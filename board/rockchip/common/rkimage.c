...
extern uint32 SecureBootSignCheck(uint8 * rsaHash, uint8 * Hash,
				  uint8 length);
...
int loadImage(uint32 offset, unsigned char *load_addr, size_t * image_size)
{
	...
	*image_size = image->size;
	...
	ftl_memcpy((void *)load_addr, image->image,
		   RK_BLK_SIZE - head_offset);

	//read the rest blks.
	blocks = DIV_ROUND_UP(*image_size, RK_BLK_SIZE);
	if (CopyFlash2Memory
	    ((uint32) load_addr + RK_BLK_SIZE - head_offset, offset + 1,
	     blocks - 1) != 0) {
...
int loadRkImage(struct fastboot_boot_img_hdr *hdr,
		fbt_partition_t * boot_ptn, fbt_partition_t * kernel_ptn)
{
	size_t image_size;
	...
	if (loadImage
	    (kernel_ptn->offset, (unsigned char *)hdr->kernel_addr,
	     &image_size) != 0) {
	...
	for (i = 0; i < header->total_chunks; i++) {
		...
		case CHUNK_TYPE_RAW:
			...
			FBTDBG("sparse: RAW blk=%d bsz=%d:"
			       ...
			       chunk->chunk_sz, header->blk_sz, sector,
			       clen);

			if (chunk->total_sz !=
			    (clen + sizeof(chunk_header_t))) {
			...
			if (outlen > section_size) {
				printf
				    ("sparse: section size %llu MB limit:"
				     " exceeded\n",
				     section_size / (1024 * 1024));
				return 1;
			}
...
int handleFlash(fbt_partition_t * ptn, void *image_start_ptr,
		loff_t d_bytes)
{
	unsigned blocks;
	if (!ptn)
	...
	return CopyMemory2Flash((uint32) image_start_ptr, ptn->offset,
				blocks);
}
...
// µÃµ½FlashData/FlashBootÊý¾Ý¿éµÄÐÅÏ¢
	for (i = 0; i < boot_hdr->ucLoaderEntryCount; i++) {
		...
		pEntry =
		    (PSTRUCT_RKBOOT_ENTRY) (old_loader +
					    boot_hdr->dwLoaderEntryOffset +
					    (boot_hdr->ucLoaderEntrySize *
					     i));
		char name[10] = "";
...
// ¹¹ÔìÐÂµÄLoaderÊý¾Ý£¬ÒÔ´«¸øLoader½øÐÐ±¾µØÉý¼¶
	...
	new_hdr->uiFlashBootOffset =
	    new_hdr->uiFlashDataOffset + new_hdr->uiFlashDataLen;
	new_hdr->uiFlashBootLen = pFlashBootEntry->dwDataSize;
	memcpy(new_loader + new_hdr->uiFlashDataOffset,
	       old_loader + pFlashDataEntry->dwDataOffset,
	       pFlashDataEntry->dwDataSize);
	memcpy(new_loader + new_hdr->uiFlashBootOffset,
	       old_loader + pFlashBootEntry->dwDataOffset,
	       pFlashBootEntry->dwDataSize);
	*new_loader_size =
	    new_hdr->uiFlashBootOffset + new_hdr->uiFlashBootLen;
...
#define RSA_KEY_LEN    0x102	//258, public key's length
	...
	if (gDrmKeyInfo.publicKeyLen == 0) {
		FBTERR
		    ("current loader unsigned, allow flash loader anyway\n");
		return 0;
	}
#ifdef FBT_DEBUG
...
#endif
	return memcmp(buf + RSA_KEY_OFFSET, gDrmKeyInfo.publicKey,
		      RSA_KEY_LEN);
}
...
bool checkBootImageSha(rk_boot_img_hdr * boothdr)
{
	...
	int size =
	    SHA_DIGEST_SIZE >
	    sizeof(boothdr->hdr.id) ? sizeof(boothdr->hdr.
					     id) : SHA_DIGEST_SIZE;

	void *kernel_data = (void *)boothdr + boothdr->hdr.page_size;
	void *ramdisk_data =
	    kernel_data + ALIGN(boothdr->hdr.kernel_size,
				boothdr->hdr.page_size);
	void *second_data = 0;
	if (boothdr->hdr.second_size) {
		second_data =
		    kernel_data + ALIGN(boothdr->hdr.ramdisk_size,
					boothdr->hdr.page_size);
	}
	...
	SHA_update(&ctx, &boothdr->hdr.kernel_size,
		   sizeof(boothdr->hdr.kernel_size));
	SHA_update(&ctx, ramdisk_data, boothdr->hdr.ramdisk_size);
	SHA_update(&ctx, &boothdr->hdr.ramdisk_size,
		   sizeof(boothdr->hdr.ramdisk_size));
	SHA_update(&ctx, second_data, boothdr->hdr.second_size);
	SHA_update(&ctx, &boothdr->hdr.second_size,
		   sizeof(boothdr->hdr.second_size));

	//only rockchip's image do these.
	SHA_update(&ctx, &boothdr->hdr.tags_addr,
		   sizeof(boothdr->hdr.tags_addr));
	SHA_update(&ctx, &boothdr->hdr.page_size,
		   sizeof(boothdr->hdr.page_size));
	SHA_update(&ctx, &boothdr->hdr.unused,
		   sizeof(boothdr->hdr.unused));
	SHA_update(&ctx, &boothdr->hdr.name, sizeof(boothdr->hdr.name));
	SHA_update(&ctx, &boothdr->hdr.cmdline,
		   sizeof(boothdr->hdr.cmdline));
...
bool checkBootImageSign(rk_boot_img_hdr * boothdr)
{
	...
	if (!memcmp
	    (boothdr->hdr.magic, FASTBOOT_BOOT_MAGIC,
		...
		if (boothdr->signTag == SECURE_BOOT_SIGN_TAG) {
			...
			if (!SecureBootEn) {
				FBTERR
				    ("loader sign mismatch, not allowed to flash!\n");
				goto fail;
			}
			//check rsa sign here.
			if (SecureBootSignCheck
			    ((uint8 *) boothdr->rsaHash,
			     (uint8 *) boothdr->hdr.id,
...
bool checkUbootImageSha(second_loader_hdr * hdr)
{
	...
	int size =
	    SHA_DIGEST_SIZE >
	    hdr->hash_len ? hdr->hash_len : SHA_DIGEST_SIZE;

	FBTDBG("compute real sha\n");

	SHA_init(&ctx);
	SHA_update(&ctx, (void *)hdr + sizeof(second_loader_hdr),
		   hdr->loader_load_size);
		  ...
		  struct cmd_fastboot_interface *priv)
{
	if (!strcmp(PARAMETER_NAME, name)) {
		...
		for (i = 0; i < PARAMETER_NUM; i++) {
			if (!StorageWriteLba
			    (i * PARAMETER_OFFSET, priv->transfer_buffer,
		...
		int size = 0;
		if (make_loader_data
		    ((char *)priv->transfer_buffer, (char *)g_pLoader,
		     &size)) {
			printf
			    ("err! make_loader_data failed(loader's key not match)\n");
			goto fail;
		}
		...
		if (!strcmp(priv->pending_ptn->name, name)
		    && priv->d_status > 0)
			goto ok;
		goto fail;
		...
		//flash boot/recovery.
		if (!checkBootImageSign
		    ((rk_boot_img_hdr *) priv->transfer_buffer)) {
			goto fail;
		}
	} else if (!strcmp(priv->pending_ptn->name, UBOOT_NAME)) {
		//flash uboot
		if (!checkUbootImageSign
		    ((second_loader_hdr *) priv->transfer_buffer)) {
			goto fail;
		}
	}
	//normal case
			 ...
			 int length, struct cmd_fastboot_interface *priv)
{
	...
	FBTDBG("direct download, size:%d, offset:%lld, rest:%lld\n",
	       size, priv->d_direct_offset,
	       priv->d_direct_size - write_len);

	if (StorageWriteLba
	    (priv->d_direct_offset + priv->pending_ptn->offset,
	     buffer + priv->transfer_buffer_pos, blocks, 0)) {
			 ...
			 int length, struct cmd_fastboot_interface *priv)
{
	...
	chunk_header_t *chunk =
	    (chunk_header_t *) calloc(sizeof(chunk_header_t), 1);
	...
	while (priv->sparse_cur_chunk < header->total_chunks) {
		...
		memcpy(chunk, buffer + priv->transfer_buffer_pos,
