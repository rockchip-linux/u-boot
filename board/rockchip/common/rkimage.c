...
static int unsparse(unsigned char *source, lbaint_t sector, lbaint_t size_kb)
{
...
int handleFlash(fbt_partition_t * ptn, void *image_start_ptr, loff_t d_bytes)
{
	...
	//base on ti's common/cmd_fastboot.c
	if (((sparse_header_t *) image_start_ptr)->magic == SPARSE_HEADER_MAGIC) {
		FBTDBG("fastboot: %s is in sparse format\n", ptn->name);
		return unsparse(image_start_ptr, ptn->offset, ptn->size_kb);
	}
...
bool checkBootImageSha(rk_boot_img_hdr * boothdr)
{
	...
	int size =
	    SHA_DIGEST_SIZE >
	    sizeof(boothdr->hdr.id) ? sizeof(boothdr->hdr.id) : SHA_DIGEST_SIZE;
...
bool checkUbootImageSha(second_loader_hdr * hdr)
{
	...
	SHA_CTX ctx;
	int size =
	    SHA_DIGEST_SIZE > hdr->hash_len ? hdr->hash_len : SHA_DIGEST_SIZE;
		  ...
		  struct cmd_fastboot_interface *priv)
{
	if (!strcmp(PARAMETER_NAME, name)) {
		...
		int size = 0;
		if (make_loader_data
		    ((char *)priv->transfer_buffer, (char *)g_pLoader, &size)) {
			printf
