...
uint32 SecureBootCheck(void)
{
...
#endif
			if (gDrmKeyInfo.publicKeyLen == 0) {	//没有公钥，是第一次才开启keyBoxEnable,
				gDrmKeyInfo.publicKeyLen = 0x100;
				ftl_memcpy(gDrmKeyInfo.publicKey, RSK_KEY,
					   0x104);
				updataFlag = 1;
				gDrmKeyInfo.drmKeyLen = 0;
				...
				gDrmKeyInfo.secureBootLock = 1;
#endif
			} else if (memcmp(gDrmKeyInfo.publicKey, RSK_KEY, 0x100) != 0) {	//如果已经存在公钥，并且公钥被替换了，那么关闭
				if (memcmp
				    (gDrmKeyInfo.publicKey + 4, RSK_KEY,
				     0x100) == 0) {
					ftl_memcpy(gDrmKeyInfo.publicKey,
						   RSK_KEY, 0x104);
					updataFlag = 1;
				} else {
	...
	if (StorageSysDataLoad(0, &gBootConfig) == FTL_OK) {
		...
		if (updataFlag) {
			updataFlag = 0;
			if (FTL_ERROR ==
			    StorageSysDataStore(0, &gBootConfig)) {
				;	// TODO:SysDataStore异常处理
			}
		}
	} else {
...
void SecureBootUnlock(uint8 * pKey)
{
	...
	if (rsaCheckMD5
	    (pKey, pKey + 256, (uint8 *) gDrmKeyInfo.publicKey,
	     128) == 0) {
...
void FlashSramLoadStore(void *pBuf, uint32 offset, uint32 dir,
			uint32 length)
{
...
#ifdef RK_SD_BOOT
	PRINT_E("sdpart %x %x %x\n", gBootConfig.sdPartOffset,
		gBootConfig.bootMedia, gBootConfig.sdSysPartOffset);
#endif
