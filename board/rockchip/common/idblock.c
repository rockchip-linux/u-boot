				 ...
				 IN OUT DRM_BYTE * pbBuffer)
{
	...
	while (cbBuffer--) {
		...
		bTemp2 ^=
		    GET_BYTE(p,
			     (GET_BYTE(p, i) + bTemp1) & (RC4_TABLESIZE - 1));
...
int get_bootloader_ver(char *boot_ver)
{
	...
	if (*(uint32 *) buf == 0xfcdc8c3b) {
		...
		sprintf(bootloader_ver, "%04X-%02X-%02X#%X.%02X",
			...
			(uint8) ((internal_boot_bloader_ver >> 8) & 0x00FF),
...
int FindIDBlock(FlashInfo * pFlashInfo, int iStart, int *iPos)
{
	...
	int i =
	    FindSerialBlocks(pFlashInfo->BlockState,
			     MAX_BLOCK_SEARCH /*MAX_BLOCK_STATE */ , iStart, 1);
	*iPos = 0;
//      PRINT_I("i = %d \n", i);
	if (i < 0) {
		return -1;
	...
	for (; i < MAX_BLOCK_SEARCH /*MAX_BLOCK_STATE */ ;
	     i =
	     FindSerialBlocks(pFlashInfo->BlockState,
			      MAX_BLOCK_SEARCH /*MAX_BLOCK_STATE */ , i + 1,
			      1)) {
//          PRINT_I("i = %d \n", i);
		if (i < 0)
			break;
	...
	*needIdSectorNum = 4 + pSec0->ucFlashBootSize;
	...
	pSec1->usLoaderDate =
	    (INT2BCD(hdr->tmCreateTime.usMonth) << 8) | INT2BCD(hdr->
								...
								usDate);
	pSec1->usLoaderVer =
	    (INT2BCD(hdr->uiMajorVersion) << 8) | INT2BCD(hdr->uiMinorVersion);
...
int get_rk28boot(uint8 * pLoader, bool dataLoaded)
{
	...
	if (!dataLoaded) {
		if (StorageReadLba(misc_part->offset + 96, (void *)pLoader, 4)
		    != 0) {
	...
	nBootSize = HEADINFO_SIZE
	    + hdr->uiUsbDataLen
	    + hdr->uiUsbBootLen + hdr->uiFlashDataLen + hdr->uiFlashBootLen;
...
// true  成功
// false 失败
bool WriteXIDBlock(USHORT * pSysBlockAddr, int iIDBCount, UCHAR * idBlockData,
		   UINT uiIdSectorNum)
{
		...
		while (uiNeedWriteSector > 0) {
			...
			iCMDRet =
			    StorageWritePba(sysSectorAddr +
					    iCount * MAX_WRITE_SECTOR, writeBuf,
					    uiWriteSector);
			...
			iCMDRet =
			    StorageReadPba(sysSectorAddr +
					   iCount * MAX_WRITE_SECTOR, readBuf,
					   uiWriteSector);
			if (ERR_SUCCESS != iCMDRet) {
