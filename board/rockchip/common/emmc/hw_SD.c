...
static void _SD_SwitchFunction(pSDM_CARD_INFO_T pCard) 
{
	...
	uint8 *pDataBuf = (uint8 *) data;
	    ...
	    SDC_SendCommand(pCard->cardId,
			    ...
			    (pCard->rca << 16), &status);
			      ...
			      (SDA_SEND_SCR | SD_READ_OP | SD_RSP_R1 |
			       WAIT_PREV), 0, &status, (64 >> 3), (64 >> 3),
	...
	do
			...
			"SD_SwitchFunction:Host bus width error\n", wide);
		    ...
		    if ((wide >= BUS_WIDTH_4_BIT) && (pDataBuf[6] & 0x4))	//bit 50:whether wide bus support
		{
			    ...
			    SDC_SendCommand(pCard->cardId,
					    (SDA_SET_BUS_WIDTH | SD_NODATA_OP |
					     SD_RSP_R1 | WAIT_PREV), 2,
					    &status);
			    ...
			    SDC_SetHostBusWidth(pCard->cardId, BUS_WIDTH_4_BIT);
	    ...
	    if (pCard->specVer >= SD_SPEC_VER_11)
				      ...
				      (SD2_SWITCH_FUNC | SD_READ_OP | SD_RSP_R1
				       | WAIT_PREV),
		...
		 if ((pDataBuf[50] & 0x2) && ((pDataBuf[47] & 0xF) == 0x1))	//bit 401:High Speed support//bit 379-376:whether function can be switched
		{
			     ...
			     || ((pDataBuf[47] == 0x1) && (!(pDataBuf[35] & 0x2))))	//bit 273 defined:check whether High Speed ready
			{
				       ...
				       &&((pDataBuf[49] & 0xF0) != 0xF0))
					    ...
					    SDC_UpdateCardFreq(pCard->cardId,
							       SDHC_FPP_FREQ);
...
void SD1X_Init(void *pCardInfo) 
{
	    ...
	    for (i = 0; i < ((FOD_FREQ * 1000) / ((48 + 48 + 2) << 1)); i++)	//按照协议的规定，这里最长可以有1s的时间，我们根据每条命令执行的时间，把它换算成循环次数
	{
		    ...
		    _Identify_SendAppCmd(pCard->cardId, 0,
					 (SDA_SD_APP_OP_COND | SD_NODATA_OP |
					  SD_RSP_R3 | WAIT_PREV), 0x00ff8000,
					 &status, 0, 0, NULL);
	...
	do
		    ...
		    _Identify_SendCmd(pCard->cardId,
				      (SD_SEND_RELATIVE_ADDR | SD_NODATA_OP |
				       SD_RSP_R6 | WAIT_PREV), 0, &status, 0, 0,
				      NULL);
	    ...
	    (pCard->tran_speed >
	     SD_FPP_FREQ) ? SD_FPP_FREQ : (pCard->tran_speed);
    ...
    /**************************************************/ 
	    ...
	    _Identify_SendAppCmd(pCard->cardId, rca,
				 (SDA_SET_CLR_CARD_DETECT | SD_NODATA_OP |
				  SD_RSP_R1 | WAIT_PREV), 0, &status, 0, 0,
				 NULL);
