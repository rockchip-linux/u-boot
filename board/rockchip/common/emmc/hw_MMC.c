...
static void _MMC_DecodeCSD(uint32 * pCSD, pSDM_CARD_INFO_T pCard) 
{
	...
	uint32 transfer_rate_unit[4] = {
	10, 100, 1000, 10000};
	uint32 time_value[16] = {
	10 /*reserved */ , 10, 12, 13, 15, 20, 25, 30, 35, 40, 45, 50,
		    55, 60, 70, 80};
	...
	pCard->capability =
	    (((c_size +
	       1) * (0x1 << (c_size_mult + 2)) * (0x1 << read_bl_len)) >> 9);
...
static int32 _SetBootSize(int32 cardId, uint32 boot_size) 
{
	    ...
	    SDC_SendCommand(cardId,
			    (SD_CMD62 | SD_NODATA_OP | SD_RSP_R1B | WAIT_PREV),
			    0xEFAC62EC, &status);
	...
	do
		...
		if (value)
		    ...
		    printf("mmc Ext_csd, ret=%x ,\n Ext[226]=%x, bootSize=%x, \n \
			   ...
			   \ret, pDataBuf[226], pCard->bootSize, \pDataBuf[215], pDataBuf[214], pDataBuf[213], pDataBuf[212],
			   value);
...
void MMC_Init(void *pCardInfo) 
{
    ...
    /**************************************************/ 
	    ...
	    _Identify_SendCmd(pCard->cardId,
			      (MMC_SET_RELATIVE_ADDR | SD_NODATA_OP | SD_RSP_R1
			       | WAIT_PREV), (rca << 16), &status, 0, 0, NULL);
	    ...
	    _Identify_SendCmd(pCard->cardId,
			      (SD_SEND_CSD | SD_NODATA_OP | SD_RSP_R2 |
			       WAIT_PREV), (rca << 16), longResp, 0, 0, NULL);
    ...
    /**************************************************/ 
	    ...
	    _Identify_SendCmd(pCard->cardId,
			      (SD_SELECT_DESELECT_CARD | SD_NODATA_OP |
			       SD_RSP_R1B | WAIT_PREV), (rca << 16), &status, 0,
			      0, NULL);
