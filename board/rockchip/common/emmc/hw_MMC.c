...
static void _MMC_DecodeCSD(uint32 * pCSD, pSDM_CARD_INFO_T pCard)
{
	...
	uint32 time_value[16] =
	    { 10 /*reserved */ , 10, 12, 13, 15, 20, 25, 30, 35, 40, 45,
50, 55, 60, 70, 80 };
...
static int32 _SetBootSize(int32 cardId, uint32 boot_size)
{
	...
	ret =
	    SDC_SendCommand(cardId,
			    (SD_CMD62 | SD_NODATA_OP | SD_RSP_R1B |
			     WAIT_PREV), 0xEFAC62EC, &status);
	if (SDC_SUCCESS != ret) {
		return ret;
	}
	...
	do {
		...
		printf
		    ("mmc Ext_csd, ret=%x ,\n Ext[226]=%x, bootSize=%x, \n \
		     ...
		     ret, pDataBuf[226], pCard->bootSize, pDataBuf[215], pDataBuf[214], pDataBuf[213], pDataBuf[212],
		     value);
#endif
		...
		eMMC_printk(5, "%s...%d...Begin to Set the bus_width.\n",
			    __FUNCTION__, __LINE__);
		Assert((wide != BUS_WIDTH_INVALID),
		       "MMC_SwitchFunction:Host bus width error\n", wide);
		...
		if (wide == BUS_WIDTH_8_BIT) {
			...
			pDataBuf[7] = 0xAA;
			ret = SDC_BusRequest(pCard->cardId,
					     (MMC4_BUSTEST_W | SD_WRITE_OP
					      | SD_RSP_R1 | WAIT_PREV), 0,
					     &status, 8, 8, pbuf);
			pDataBuf[0] = 0x00;
			pDataBuf[1] = 0x00;
			pDataBuf[2] = 0x00;
		...
		if (wide == BUS_WIDTH_4_BIT) {
			...
			if (pDataBuf[0] != 0xA5) {
				SDC_SetHostBusWidth(pCard->cardId,
						    BUS_WIDTH_1_BIT);
				break;
			}
		...
		ret = SDC_SendCommand(pCard->cardId,
				      (MMC4_SWITCH_FUNC | SD_NODATA_OP |
				       SD_RSP_R1B | WAIT_PREV),
				      ((0x3 << 24) | (183 << 16) |
				       (value << 8)), &status);
...
//    uint8            tmp;
	...
	ret = SDC_SendCommand(pCard->cardId,
			      (MMC4_SWITCH_FUNC | SD_NODATA_OP | SD_RSP_R1B
			       | WAIT_PREV),
...
//注意:
/****************************************************************/
int32 MMC_SetBootBusWidth(void *pCardInfo, bool enable,
			  HOST_BUS_WIDTH_E width)
{
	pSDM_CARD_INFO_T pCard;
	uint32 status = 0;
	int32 ret = SDM_SUCCESS;
...
void MMC_Init(void *pCardInfo)
{
	...
	for (i = 0; i < ((FOD_FREQ * 1000) / (48 + 2)); i++) {
		ret =
		    _Identify_SendCmd(pCard->cardId,
				      (MMC_SEND_OP_COND | SD_NODATA_OP |
				       SD_RSP_R3 | WAIT_PREV), 0x40ff8000,
				      &status, 0, 0, NULL);
		if (SDC_SUCCESS == ret) {
			if (status & 0x80000000) {
				if ((0x80ff8000 == status)
				    || (0x80ff8080 == status)) {
					type = MMC;
					break;
				} else if ((0xc0ff8000 == status)
					   || (0xc0ff8080 == status)) {
					type = eMMC2G;
					break;
				} else {
					gSDMDriver[pCard->cardId].step =
					    0x41;
					gSDMDriver[pCard->cardId].error =
					    status;
	...
	ret =
	    _Identify_SendCmd(pCard->cardId,
			      (SD_ALL_SEND_CID | SD_NODATA_OP | SD_RSP_R2 |
			       WAIT_PREV), 0, longResp, 0, 0, NULL);
	if (SDC_SUCCESS != ret) {
	...
	ret =
	    _Identify_SendCmd(pCard->cardId,
			      (MMC_SET_RELATIVE_ADDR | SD_NODATA_OP |
			       SD_RSP_R1 | WAIT_PREV), (rca << 16),
			      &status, 0, 0, NULL);
	if (SDC_SUCCESS != ret) {
	...
	ret =
	    _Identify_SendCmd(pCard->cardId,
			      (SD_SEND_CSD | SD_NODATA_OP | SD_RSP_R2 |
			       WAIT_PREV), (rca << 16), longResp, 0, 0,
			      NULL);
	if (SDC_SUCCESS != ret) {
	...
	pCard->tran_speed =
	    (pCard->tran_speed >
	     MMC_FPP_FREQ) ? MMC_FPP_FREQ : (pCard->tran_speed);
	...
	ret =
	    _Identify_SendCmd(pCard->cardId,
			      (SD_SELECT_DESELECT_CARD | SD_NODATA_OP |
			       SD_RSP_R1B | WAIT_PREV), (rca << 16),
			      &status, 0, 0, NULL);
	if (SDC_SUCCESS != ret) {
	...
	eMMC_printk(5,
		    "%s  %d    MMC Init over, then begin to run SwitchFunction \n",
		    __FILE__, __LINE__);
