...
RK_ALIGN(64)
uint32 SpareBuf[(32 * 8 * 4 / 4)];
RK_ALIGN(64)
uint32 emmcData[(1024 * 8 * 4 / 4)];
...
void FTLTest(uint8 ChipSel)
{
	...
	printf("----->write start=%x end=%x time=%d", start, end,
	...
	printf("----->read start=%x end=%x time=%d", start, end,
	       start > end ? start - end : end - start);
#if 0
	TestCountNUM = TestStartSector;
	for (i = 0; i <= TestFileLen / 512 / 64; i++) {
		...
		for (j = 0; j < 64 * 256; j++) {
			if (j == 0) {
				if (pread[0] != i) {
					PRINT_E
					    ("write not match:row=%x, num=%x, write=%x, read=%x\n",
	...
	for (loop = 0; loop < 10; loop++) {
		...
		for (TestLBA = TestStartSector + loop; (TestLBA + TestSecCount) < TestEndLBA;)	//SysAreaBlock
		{
			...
			for (j = 0; j < TestSecCount * 256; j++) {
				if (pwrite[j] != pread[j]) {
					PRINT_E
					    ("check not match:row=%x, num=%x, write=%x, read=%x\n",
					     TestLBA, j, pwrite[j],
					     pread[j]);
					break;
				}
...
uint32 SdmmcInit(uint32 ChipSel)
{
	...
	if (ret1 == SDM_SUCCESS)	//卡能识别
	{
		...
		ret1 =
		    SDM_IOCtrl(SDM_IOCTR_GET_BOOT_CAPABILITY, ioctlParam);
		...
		if (gSdCardInfoTbl[ChipSel].BootCapSize) {
			...
			ret1 =
			    SDM_IOCtrl(SDM_IOCTR_ACCESS_BOOT_PARTITION,
				       ioctlParam);
			if (ret1 == SDM_SUCCESS) {
			}
			SDM_Read(ChipSel, 0, 4, gIdDataBuf);	// id blk data
			EmmcSetBootPart(ChipSel, gEmmcBootPart,
					EMMC_DATA_PART);
			SDM_Read(ChipSel, SD_CARD_BOOT_PART_OFFSET, 4, emmcData);	// id blk data
			if (emmcData[0] == 0xFCDC8C3B)
				gSdCardInfoTbl[ChipSel].FwPartOffset =
				    SD_CARD_FW_PART_OFFSET;
		} else {
		...
		if (gSdCardInfoTbl[ChipSel].FwPartOffset ==
		    SD_CARD_FW_PART_OFFSET) {
			SDM_Read(ChipSel, SD_CARD_SYS_PART_OFFSET, 4,
				 gSysData);
		}
#ifdef FTL_DEBUG
		FTLTest(ChipSel);
...
uint32 SdmmcBootWritePBA(uint8 ChipSel, uint32 PBA, void *pbuf,
			 uint16 nSec)
{
	uint32 i;
	uint16 len;
	...
	if (gSdCardInfoTbl[ChipSel].BootCapSize > 0) {
		...
		{
			//写第二份数据，从第一个boot区引导
			EmmcSetBootPart(ChipSel, EMMC_BOOT_PART,
					EMMC_BOOT_PART2);
			PBA &= (EMMC_BOOT_PART_SIZE - 1);
		} else {
			//写第一份数据，从第二个boot区引导
			EmmcSetBootPart(ChipSel, EMMC_BOOT_PART2,
					EMMC_BOOT_PART);
		}
	} else {
		if (gSdCardInfoTbl[ChipSel].FwPartOffset ==
	...
	for (len = 0; len < nSec; len += pageSizeLimit) {
		for (i = 0; i < (MIN(nSec, pageSizeLimit)); i++) {
			ftl_memcpy(emmcData + i * 128,
				   pDataBuf + (len + i) * 132, 512);
			ftl_memcpy(SpareBuf + i * 4,
				   pDataBuf + (len + i) * 132 + 128, 16);
		}
		SDM_Write(ChipSel, PBA, (MIN(nSec, pageSizeLimit)),
			  emmcData);
	}
...
uint32 SdmmcBootReadPBA(uint8 ChipSel, uint32 PBA, void *pbuf, uint16 nSec)
{
	...
	for (len = 0; len < nSec; len += pageSizeLimit) {
		SDM_Read(ChipSel, PBA, (MIN(nSec, pageSizeLimit)),
			 emmcData);
		for (i = 0; i < (MIN(nSec, pageSizeLimit)); i++) {
			ftl_memcpy(pDataBuf + (len + i) * 132,
				   emmcData + (i) * 128, 512);
			ftl_memcpy(pDataBuf + (len + i) * 132 + 128,
				   SpareBuf + (i) * 4, 16);
		}
	}
...
uint32 SdmmcBootReadLBA(uint8 ChipSel, uint32 LBA, void *pbuf, uint16 nSec)
{
	...
	iret =
	    SDM_Read(ChipSel, LBA + gSdCardInfoTbl[ChipSel].FwPartOffset,
		     nSec, pbuf);
	if (iret != FTL_OK)
...
uint32 SdmmcBootWriteLBA(uint8 ChipSel, uint32 LBA, void *pbuf,
			 uint16 nSec, uint16 mode)
{
	uint32 iret = FTL_OK;
	if (gSdCardInfoTbl[ChipSel].BootCapSize > 0) {
		EmmcSetBootPart(ChipSel, EMMC_BOOT_PART, EMMC_DATA_PART);
	...
	iret =
	    SDM_Write(ChipSel, LBA + gSdCardInfoTbl[ChipSel].FwPartOffset,
		      nSec, pbuf);
	if (iret != FTL_OK)
...
{
	return gSdCardInfoTbl[ChipSel].UserCapSize -
	    gSdCardInfoTbl[ChipSel].FwPartOffset;
}
...
uint32 SdmmcSysDataLoad(uint8 ChipSel, uint32 Index, void *Buf)
{
	...
	if (gSdCardInfoTbl[ChipSel].FwPartOffset == SD_CARD_FW_PART_OFFSET) {
		...
		ret =
		    SDM_Read(ChipSel, SD_CARD_SYS_PART_OFFSET + Index, 1,
			     &tmpBuf);
...
uint32 SdmmcSysDataStore(uint8 ChipSel, uint32 Index, void *Buf)
{
	...
	if (gSdCardInfoTbl[ChipSel].FwPartOffset == SD_CARD_FW_PART_OFFSET) {
		...
		ret =
		    SDM_Write(ChipSel, SD_CARD_SYS_PART_OFFSET + Index, 1,
			      &tmpBuf);
...
调用函数:无
***************************************************************************/
uint32 SdmmcBootErase(uint8 ChipSel, uint32 blkIndex, uint32 nblk,
		      uint8 mod)
{
	return OK;
}
...
void EmmcIdBlockRecover(uint8 part)
{
	...
	if (gEmmcBootPart != part
	    && gSdCardInfoTbl[EMMC_CARD_ID].BootCapSize > 0) {
		uint32 ioctlParam[5] = { 0, 0, 0, 0, 0 };
		...
		SDM_IOCtrl(SDM_IOCTR_ACCESS_BOOT_PARTITION, ioctlParam);
		if (SDM_Read
		    (EMMC_CARD_ID, 0, EMMC_BOOT_PART_SIZE,
		     IDBlockData1) != SDM_SUCCESS) {
			return;
		}
		...
		SDM_IOCtrl(SDM_IOCTR_ACCESS_BOOT_PARTITION, ioctlParam);
		SDM_Read(EMMC_CARD_ID, 0, EMMC_BOOT_PART_SIZE,
			 IDBlockData0);
		if (memcmp(IDBlockData1, IDBlockData0, IDblockDataSize) ==
		    0) {
...
int sdBootCheckSdCard(uint8 ChipSel)
{
	...
	if (gSdCardInfoTbl[ChipSel].FwPartOffset == SD_CARD_FW_PART_OFFSET) {
		...
		{
			gSdCardInfoTbl[ChipSel].UserPartOffset =
			    ((uint32) (buf[0xE4]) << 16) | buf[0xE3];
			gSdCardInfoTbl[ChipSel].UserPartSize =
			    ((uint32) (buf[0xE6]) << 16) | buf[0xE5];
			if (gSdCardInfoTbl[ChipSel].UserPartOffset <
			    gSdCardInfoTbl[ChipSel].FwPartOffset)
				return -2;
			gSdCardInfoTbl[ChipSel].FwPartOffset =
			    SD_CARD_FW_PART_OFFSET;
