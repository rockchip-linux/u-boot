...
int rpmb_send_cmd(struct mmc *mmc, struct mmc_cmd *cmd, struct mmc_data *data)
{
	...
	if (MMC_RSP_R1 == cmd->resp_type)
        	resp_type = SD_RSP_R1;
    	else if (MMC_RSP_R1b == cmd->resp_type)
        	resp_type = SD_RSP_R1B;
    	else {
		debug("%s  %d  resp_type error\n",__FILE__,__LINE__);
		return -1;
    	}
		...
		ret = SDC_BusRequest(2, \
		                     cmd->cmdidx | flags | resp_type | WAIT_PREV, \
		                     cmd->cmdarg, \
		                     &cmd->response[0], \
		                     data->blocksize,
		...
		ret = SDC_SendCommand(2, \
					(cmd->cmdidx | SD_NODATA_OP | resp_type | WAIT_PREV), \
					cmd->cmdarg, \
					&cmd->response[0]);
    	}
...
int rpmb_switch_part(int dev_num, unsigned int part_num)
{
	int  ret = 0;
	uint32 status = 0;
	pSDM_CARD_INFO_T pCard = &gSDMDriver[dev_num].cardInfo;

	ret = SDC_SendCommand(dev_num, \
					 (MMC4_SWITCH_FUNC | SD_NODATA_OP | SD_RSP_R1B | WAIT_PREV), \
					  ((0x3 << 24) | (179 << 16) | ((part_num & PART_ACCESS_MASK) << 8)), \
					 &status);
			      ...
			      bool is_rel_write)
{
	...
	return rpmb_send_cmd(mmc, &cmd, NULL);
}
			    ...
			    unsigned int count, bool is_rel_write)
{
	...
	ret = rpmb_send_cmd(mmc, &cmd, &data);
	if (ret) {
#ifdef CONFIG_MMC_RPMB_TRACE
...
static int mmc_rpmb_response(struct mmc *mmc, struct s_rpmb *s, unsigned short expected, unsigned short cnt)
{
	struct mmc_cmd cmd = {0};
	struct mmc_data data;
	int ret;
...
int init_rpmb(void)
{
	...
	if (rpmb_switch_part(2, MMC_PART_RPMB) != 0)
...
{
	return read_counter(NULL, requestpackets);
}

int do_programkey(struct s_rpmb *requestpackets)
{
	return program_key(NULL, requestpackets);
}

int do_authenticatedread(struct s_rpmb *requestpackets, uint16_t block_count)
{
	return authenticated_read(NULL, requestpackets, block_count);
}

int do_authenticatedwrite(struct s_rpmb *requestpackets)
{
	return authenticated_write(NULL, requestpackets);
}

struct mmc *do_returnmmc(void)
{
	pSDM_CARD_INFO_T pCard = &gSDMDriver[2].cardInfo;

	memcpy(g_rpmb_mmc.cid, pCard->cid, 16);
	g_rpmb_mmc.capacity_rpmb = pCard->capacity_rpmb;
