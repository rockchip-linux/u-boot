			     ...
			     uint32 CmdArg,  void *pDataBuf, uint32 DataLen) 
{
	...
	pReg->SDMMC_IDINTEN = IDMAC_EN_INT_ALL;
	    ...
	    pSDC->ErrorStat = SDC_SUCCESS;
	     ...
	     SDC_StartCmd(pReg, (cmd & ~(RSP_BUSY)) | START_CMD | USE_HOLD_REG))
...
static int32 _ChangeFreq(SDMMC_PORT_E nSDCPort, uint32 freqKHz) 
{
	...
	if (freqKHz < 12000)	//低频下, 外面供给的clk就不能太高,不然cmd和数据的hold time不够
	{
	    ...
	    (secondFreq / freqKHz) + (((secondFreq % freqKHz) > 0) ? 1 : 0);
...
static int32 _ChangeFreq(SDMMC_PORT_E nSDCPort, uint32 freqKHz) 
{
	...
	 if (freqKHz == 0)	//频率不能为0，否则后面会出现除数为0
	{
	    ...
	    if (freqKHz <= 400)
		    ...
		    ahbFreq / freqKHz + (((ahbFreq % freqKHz) > 0) ? 1 : 0);
				  ...
				  uint32 dataLen, pFunc cb) 
{
	...
	volatile uint32 *pFIFO = pSDCFIFOADDR(nSDCPort);
	    ...
	    if (count <= FIFO_DEPTH)
		       ...
		       (nSDCPort, (uint32) pFIFO, (uint32) pBuf, count, 1, cb))
   ...
   */ 
	...
	if (count > (RX_WMARK + 1))	//当总的数据长度小于等于RX_WMARK+1时，不会向DMA发起请求(这个跟datasheet说的不大一样)
	{
		     ...
		     (nSDCPort, (uint32) pDataBuf,
		      (uint32) pSDCFIFOADDR(nSDCPort), (dataLen >> 2), 0, cb))
...
 int eMMC_Switch_ToMaskRom(void) 
{
			   ...
			   (SD_GO_IDLE_STATE | SD_NODATA_OP | SD_RSP_NONE |
			    NO_WAIT_PREV | SEND_INIT), 0xF0F0F0F0, NULL, 0, 0,
			   NULL);
...
//    uint8   *tempbuf;
		...
		(((value = pReg->SDMMC_STATUS) & 0x3FFE0000)
		 && (!(cmd & STOP_CMD))), "SDC_BusRequest:+FIFO not empty\n",
		value);
		...
		if (dataLen <= MAX_DATA_SIZE_IDMAC && dataLen >= 512)
			...
			return SDC_RequestIDMA(cardId, cmd, cmdArg, pDataBuf,
						dataLen);
	...
	while (!((value = pReg->SDMMC_RINISTS) & CD_INT) && (timeOut > 0))
		...
		while (!((value = pReg->SDMMC_RINISTS) & (DTO_INT | SBE_INT))
		       && timeOut)
