...
static void SDC_HandleIDSTS(SDMMC_PORT_E SDCPort)
{
	...
	if (idsts & IDMAC_TI) {	// Tx Interrupt
...
Log:
*/
static int32 SDC_SetIDMADesc(SDMMC_PORT_E SDCPort, uint32 buffer,
			     uint32 BufSize)
{
	pSDC_INFO_T pSDC = &gSDCInfo[SDCPort];
	pSDC_REG_T pReg = pSDCReg(SDCPort);
	...
	for (i = 0; i < MAX_DESC_NUM_IDMAC; i++, pDesc++) {
		...
		pDesc->desc1 =
		    ((size << DescBuf1SizeShift) & DescBuf1SizMsk);
		pDesc->desc2 = (uint32) buffer;
		pDesc->desc0 =
		    DescSecAddrChained | DescOwnByDma | ((i == 0) ?
							 DescFirstDesc : 0)
		    | DescDisInt;
...
static int32 SDC_RequestIDMA(SDMMC_PORT_E SDCPort,
			     uint32 cmd,
			     uint32 CmdArg, void *pDataBuf, uint32 DataLen)
{
	pSDC_INFO_T pSDC = &gSDCInfo[SDCPort];
	pSDC_REG_T pReg = pSDCReg(SDCPort);
	uint32 SlaveIntMask;
	...
	if (SDC_SUCCESS !=
	    SDC_StartCmd(pReg,
			 (cmd & ~(RSP_BUSY)) | START_CMD | USE_HOLD_REG)) {
		return SDC_SDC_ERROR;
	}
	...
	do {
		...
		if ((--timeout) == 0 || pSDC->ErrorStat != SDC_SUCCESS)
			break;
	} while ((pReg->SDMMC_RINISTS & (CD_INT | DTO_INT)) !=
		 (CD_INT | DTO_INT));
...
static int32 SDC_ResetFIFO(pSDC_REG_T pReg)
{
	...
	if (!(value & FIFO_EMPTY)) {
		...
		timeOut = 10000;
		while (((value = pReg->SDMMC_CTRL) & (FIFO_RESET))
		       && (timeOut > 0)) {
			//SDOAM_Delay(1);
			timeOut--;
		}
...
static int32 _ChangeFreq(SDMMC_PORT_E nSDCPort, uint32 freqKHz)
{
	...
	suitMmcClkDiv =
	    ahbFreq / MMCHS_52_FPP_FREQ +
	    (((ahbFreq % MMCHS_52_FPP_FREQ) > 0) ? 1 : 0);
...
static int32 _ChangeFreq(SDMMC_PORT_E nSDCPort, uint32 freqKHz)
{
	...
	for (mmcClkDiv = minMmcClkDiv; mmcClkDiv <= 8; mmcClkDiv++) {
		...
		if ((newCardFreq <= freqKHz)
		    && ((freqKHz - newCardFreq) < minFreqOffset)) {
	...
	if (freqKHz <= 400) {
		suitMmcClkDiv = 8;
		suitCclkInDiv =
		    ((SDPAM_MAX_AHB_FREQ * 1000) / (freqKHz << 3)) +
		    ((((SDPAM_MAX_AHB_FREQ * 1000) % (freqKHz << 3)) >
		      0) ? 1 : 0);
		if (((suitCclkInDiv & 0x1) == 1) && (suitCclkInDiv != 1)) {
			suitCclkInDiv++;	//除了1分频，保证是偶数倍
...
//注意:
/****************************************************************/
static int32 _PrepareForWriteData(SDMMC_PORT_E nSDCPort, void *pDataBuf,
				  uint32 dataLen, pFunc cb)
{
#if EN_SD_DMA
	volatile uint32 value = 0;
	pSDC_REG_T pReg = pSDCReg(nSDCPort);
	...
	if (count <= FIFO_DEPTH) {
		...
		if (!SDPAM_DMAStart
		    (nSDCPort, (uint32) pFIFO, (uint32) pBuf, count, 1,
		     cb)) {
...
//注意:
/****************************************************************/
static int32 _PrepareForReadData(SDMMC_PORT_E nSDCPort, void *pDataBuf,
				 uint32 dataLen, pFunc cb)
{
#if EN_SD_DMA
	volatile uint32 value = 0;
...
/*    uint8 *testbuf= (uint8 *)pDataBuf;
   ...
   */
	...
	if (count > (RX_WMARK + 1))	//当总的数据长度小于等于RX_WMARK+1时，不会向DMA发起请求(这个跟datasheet说的不大一样)
	{
		...
		gSDCInfo[nSDCPort].intInfo.pBuf = (uint32 *) pDataBuf;
		if (!SDPAM_DMAStart
		    (nSDCPort, (uint32) pDataBuf,
		     (uint32) pSDCFIFOADDR(nSDCPort), (dataLen >> 2), 0,
		     cb)) {
...
/****************************************************************/
static int32 _ReadRemainData(SDMMC_PORT_E nSDCPort, uint32 originalLen,
			     void *pDataBuf)
{
	volatile uint32 value = 0;
	volatile uint32 *pFIFO = pSDCFIFOADDR(nSDCPort);
	...
	if (!(value & FIFO_EMPTY)) {
		...
		if (count > (originalLen >> 2)) {
			Assert((!(value & FIFO_EMPTY)),
			       "_ReadRemainData:need data, but FIFO empty\n",
			       value);
			if (value & FIFO_EMPTY) {
				return SDC_SDC_ERROR;
			}
	...
	value = pReg->SDMMC_STATUS;
	if (!(value & FIFO_EMPTY)) {
		while ((gSDCInfo[nSDCPort].intInfo.transLen <
			gSDCInfo[nSDCPort].intInfo.desLen)
		       && (!(value & FIFO_EMPTY))) {
			pBuf[gSDCInfo[nSDCPort].intInfo.transLen++] =
			    *pFIFO;
			value = pReg->SDMMC_STATUS;
		}
...
//Re_read:        
	...
	if (value & TXDR_INT) {
		...
		//fill data
		pbuf32 = &p_intInfo->pBuf[p_intInfo->transLen];
		if ((p_intInfo->desLen - p_intInfo->transLen) >
		    (FIFO_DEPTH - TX_WMARK)) {
...
void SDC_Init(uint32 CardId)
{
	...
	if (CardId == 0) {
		...
		Assert((gSDCInfo[SDC0].event != NULL),
		       "SDC_Init:Create SDC0 event failed\n",
...
#error SDMMC0 Bus Width Not Support
		...
		Assert((gSDCInfo[SDC1].event != NULL),
		       "SDC_Init:Create SDC1 event failed\n",
		       (uint32) gSDCInfo[SDC1].event);
...
//eMMC
		...
		Assert((gSDCInfo[SDC2].event != NULL),
		       "SDC_Init:Create SDC1 event failed\n",
	...
	nSDCPort = CardId;
	{
		...
		clkdiv =
		    (clkvalue + (uint32) MMCHS_52_FPP_FREQ -
		     1) / (uint32) MMCHS_52_FPP_FREQ;
		if (clkdiv > 0x3f) {
			//PLL频率太大
			return;
		}
		...
		if (bPortEnable[nSDCPort]) {
			...
			while (((value =
				 pReg->
				 SDMMC_CTRL) & (FIFO_RESET | SDC_RESET))
			       && (timeOut > 0)) {
				SDOAM_Delay(1);
				timeOut--;
			}
...
int eMMC_Switch_ToMaskRom(void)
{
	uint32 value = 0;
	_Identify_SendCmd(2,
			  (SD_GO_IDLE_STATE | SD_NODATA_OP | SD_RSP_NONE |
			   NO_WAIT_PREV | SEND_INIT), 0xF0F0F0F0, NULL, 0,
			  0, NULL);
	value = BUS_1_BIT;
...
int32 SDC_ResetController(int32 cardId)
{
	...
	timeOut = 10000;
	while (((value = pReg->SDMMC_CTRL) & (FIFO_RESET | SDC_RESET))
	       && (timeOut > 0)) {
		SDOAM_Delay(1);
		timeOut--;
	}
	...
	/* config FIFO */
	pReg->SDMMC_FIFOTH =
	    (SD_MSIZE_16 | (RX_WMARK << RX_WMARK_SHIFT) |
	     (TX_WMARK << TX_WMARK_SHIFT));
...
void SDC_UpdateAhbFreq(uint32 newAhbKHz)
{
	...
	for (; nSDCPort < nSDCPortMax; nSDCPort++) {
		...
		mmc_clk_div = newAhbKHz / (gSDCInfo[nSDCPort].cardFreq)
		    +
		    (((newAhbKHz % (gSDCInfo[nSDCPort].cardFreq)) >
		      0) ? 1 : 0);
		if (mmc_clk_div > 8) {
			mmc_clk_div = 8;
		}
...
int32 SDC_ControlClock(int32 cardId, bool enable)
{
	...
	if (!enable) {
		...
		clkvalue = SDPAM_GetAHBFreq();
		clkdiv =
		    clkvalue / MMCHS_52_FPP_FREQ +
		    (((clkvalue % MMCHS_52_FPP_FREQ) > 0) ? 1 : 0);
		if (clkdiv > 0x3f) {
			//PLL频率太大
			return SDC_SDC_ERROR;
		}
...
//    uint8   *tempbuf;
	...
	if (!(cmd & STOP_CMD)) {
		...
		if (!(value & FIFO_EMPTY)) {
			...
			if (timeOut == 0) {
				eMMC_printk(3,
					    "SDC_BusRequest:  CMD=%d SDC_SDC_ERROR  %d\n",
					    cmd & 0x3f, __LINE__);
				return SDC_SDC_ERROR;
			}
		}
	...
	if (cmd & WAIT_PREV) {
		...
		if (value & DATA_BUSY) {
			eMMC_printk(3,
				    "SDC_BusRequest:  CMD=%d DATA BUSY  %d\n",
				    cmd & 0x3f, __LINE__);
			return SDC_BUSY_TIMEOUT;
		}
	}
	Assert(!
	       (((value = pReg->SDMMC_STATUS) & DATA_BUSY)
		&& (cmd & WAIT_PREV)), "SDC_BusRequest:busy error\n",
	       value);
	Assert(!
	       (((value = pReg->SDMMC_STATUS) & 0x3FFE0000)
		&& (!(cmd & STOP_CMD))),
	       "SDC_BusRequest:+FIFO not empty\n", value);
	Assert(!((value = pReg->SDMMC_CMD) & START_CMD),
	       "SDC_BusRequest:start bit != 0\n", value);
	value = pReg->SDMMC_CMD;
	if (value & START_CMD) {
		eMMC_printk(3,
			    "SDC_BusRequest:  CMD=%d START CMD ERROR %d\n",
			    cmd & 0x3f, __LINE__);
		return SDC_SDC_ERROR;
	}
	...
	if (gSDCInfo[nSDCPort].updateCardFreq) {
		...
		if (ret != SDC_SUCCESS) {
			eMMC_printk(3,
				    "SDC_BusRequest:  CMD=%d ChangeFreq ERROR %d\n",
				    cmd & 0x3f, __LINE__);
			return ret;
		}
	...
	if (cmd & DATA_EXPECT) {
		...
		if (dataLen <= MAX_DATA_SIZE_IDMAC && dataLen >= 512) {
			return SDC_RequestIDMA(cardId, cmd, cmdArg,
					       pDataBuf, dataLen);
		}
#endif
		...
		if ((cmd & SD_OP_MASK) == SD_WRITE_OP) {
			...
			_PrepareForWriteData(nSDCPort, pDataBuf, dataLen,
					     cb);
		} else {
			if (nSDCPort == SDC0) {
				cb = _SDC0DMACallback;
			...
			_PrepareForReadData(nSDCPort, pDataBuf, dataLen,
					    cb);
		}
		pReg->SDMMC_BLKSIZ = blockSize;
		pReg->SDMMC_BYTCNT = dataLen;	//这个寄存器的长度一定要设置为需要的长度，不用考虑SDMMC控制器的32bit对齐
	...
	if (timeOut == 0) {
		eMMC_printk(3,
			    "SDC_BusRequest:  CMD=%d  START_CMD timeout %d\n",
			    cmd & 0x3f, __LINE__);
		return SDC_SDC_ERROR;
	}
#if EN_SD_INT
	...
	if (timeOut == 0) {
		eMMC_printk(5,
			    "SDC_BusRequest 444-1  ---START_CMD timeout---  %s  %d\n",
			    __FILE__, __LINE__);
		return SDC_SDC_ERROR;
	}
	pReg->SDMMC_RINISTS = CD_INT;
...
#endif
		eMMC_printk(3,
			    "SDC_BusRequest:  CMD=%d  SDC_RESP_TIMEOUT %d\n",
			    cmd & 0x3f, __LINE__);
	...
	if (cmd & DATA_EXPECT) {
		...
		while (!
		       ((value =
			 pReg->SDMMC_RINISTS) & (DTO_INT | SBE_INT))
		       && timeOut) {
			SDOAM_Delay(1);
			timeOut--;
		}
		if (timeOut == 0) {
			eMMC_printk(5,
				    "SDC_BusRequest 444-1  ---wait for data timeout---  %s  %d\n",
				    __FILE__, __LINE__);
			return SDC_SDC_ERROR;
		}
		pReg->SDMMC_RINISTS = DTO_INT;
		...
		if ((cmd & SD_OP_MASK) == SD_WRITE_OP) {
			...
			if (value &
			    (SBE_INT | EBE_INT | DRTO_INT | DCRC_INT)) {
				if (value & SBE_INT) {
					uint32 stopCmd = 0;
					stopCmd =
					    (SD_STOP_TRANSMISSION |
					     SD_NODATA_OP | SD_RSP_R1 |
					     STOP_CMD | NO_WAIT_PREV);
					SDC_Start(pReg,
						  ((stopCmd &
						    (~(RSP_BUSY))) |
						   START_CMD));
					...
					if (timeOut == 0) {
						eMMC_printk(3,
							    "SDC_BusRequest:  CMD=%d START_CMD ERROR %d\n",
							    cmd & 0x3f,
							    __LINE__);
						return SDC_SDC_ERROR;
					}
#if EN_SD_INT
					SDOAM_GetEvent(gSDCInfo[nSDCPort].
						       event, CD_EVENT);
					SDOAM_GetEvent(gSDCInfo[nSDCPort].
						       event, DTO_EVENT);
#else
				...
				ret =
				    _ReadRemainData(nSDCPort, dataLen,
						    pDataBuf);
			}
	...
	if (ret == SDC_SUCCESS) {
		if (cmd & RSP_BUSY)	//R1b
		{
			...
			if (ret != SDC_SUCCESS) {
				eMMC_printk(3,
					    "SDC_BusRequest:  CMD=%d WaitCard Ready Timeout%d\n",
					    cmd & 0x3f, __LINE__);
				return ret;
			}
		}
