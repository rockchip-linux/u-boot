...
static int32 _DataErrorHandle(int32 cardId) 
{
	    ...
	    SDC_SendCommand(cardId,
			    (SD_STOP_TRANSMISSION | SD_NODATA_OP | SD_RSP_R1B |
			     STOP_CMD | NO_WAIT_PREV), 0, &status);
			 ...
			 void *pBuf) 
{
	...
	uint32 repeatCount = 0;
			 ...
			 || (ret == SDC_DATA_READ_TIMEOUT))
				      ...
				      ((ret ==
					SDC_END_BIT_ERROR) ? "SDC_END_BIT_ER" :
			  ...
			  uint8 * pNewPassword, bool lock) 
{
	...
	uint8 *pPasswordData = (uint8 *) data;
	       ...
	       ||((pCardInfo->bPassword) && (pOldPassword == NULL)))
		    ...
		    SDOAM_Strlen(pOldPassword) + SDOAM_Strlen(pNewPassword);
	...
	uint32 status = 0;
	    ...
	    SDC_SendCommand(cardId,
			    (SD_SEND_STATUS | SD_NODATA_OP | SD_RSP_R1 |
			     NO_WAIT_PREV), (pCardInfo->rca << 16), &status);
		...
		ret =
		    _Identify_SendCmd(cardId,
				      (SD_GO_IDLE_STATE | SD_NODATA_OP |
				       SD_RSP_NONE | NO_WAIT_PREV | SEND_INIT),
				      0, NULL, 0, 0, NULL);
	...
	/**************************************************/ 
		    ...
		    _Identify_SendCmd(cardId,
				      (SD2_SEND_IF_COND | SD_NODATA_OP |
				       SD_RSP_R7 | WAIT_PREV), 0x1AA, &status,
				      0, 0, NULL);
			    ...
			    uint32 dataLen, void *pDataBuf)
{
	...
	do
		    ...
		    SDC_BusRequest(cardId, cmd, cmdArg, responseBuf, blockSize,
				   dataLen, pDataBuf);
		retry--;
	}
	    while ((ret &
			    (SDC_RESP_ERROR | SDC_RESP_CRC_ERROR |
				     SDC_RESP_TIMEOUT)) && (retry > 0));
    ...
    int32 SDM_Read(int32 cardId, uint32 blockNum, uint32 blockCount, void *pBuf)
{
	...
	if (((blockNum + blockCount) > (pSDMDriver->cardInfo.capability)))	//(blockCount > (pSDMDriver->cardInfo.capability)) || 
	{
		...
		int i;
		    ...
		    //地址4字节对齐
		    ret =
		    (pSDMDriver->cardInfo.fun.read) (cardId, (blockNum << mul),
						     blockCount, pBuf);
		    ...
		    void *pBuf)
{
	...
	 if (pSDMDriver->cardInfo.WriteProt)	//只有SD卡才有写保护,MMC卡这个值总是为0
	{
		    ...
		    (pSDMDriver->cardInfo.fun.write) (cardId, (blockNum << mul),
						      blockCount, pBuf);
