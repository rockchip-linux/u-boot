...
static bool get_entry(const char *file_path, index_tbl_entry * entry)
{
	...
	if (memcmp
	    (header.magic, RESOURCE_PTN_HDR_MAGIC, sizeof(header.magic))) {
	...
	if (header.tbl_entry_num * header.tbl_entry_size <= 0xFFFF) {
		cache =
		    (char *)malloc(header.tbl_entry_num *
				   header.tbl_entry_size * BLOCK_SIZE);
		if (cache) {
			if (StorageReadLba
			    (ptn_offset + header.header_size, cache,
	...
	for (i = 0; i < header.tbl_entry_num; i++) {
		...
		if (!cache) {
			...
			memcpy(entry,
			       cache +
			       i * header.tbl_entry_size * BLOCK_SIZE,
			       sizeof(*entry));
		}

		if (memcmp
		    (entry->tag, INDEX_TBL_ENTR_TAG, sizeof(entry->tag))) {
...
bool load_content(resource_content * content)
{
	...
	if (StorageReadLba(get_ptn_offset() +
			   content->content_offset, content->load_addr,
			   blocks)) {
		       ...
		       int offset_block, void *data, int blocks)
{
	...
	if (StorageReadLba(get_ptn_offset() +
			   content->content_offset + offset_block, data,
			   blocks)) {
		return false;
	}
	return true;
...
void test_content()
{
	...
	if (!blocks) {
		...
		((char *)content.load_addr)[content.content_size - 1] =
		    '\0';
		printf("%s\n", content.load_addr);
	} else {
		...
		if (!data)
			goto end;
		if (!load_content_data
		    (&content, offset_block, data, blocks)) {
			goto end;
		}
