...
static bool parse_level_conf(const char *arg, anim_level_conf * level_conf)
{
	...
	if (buf) {
		level_conf->num =
		    atoi(buf + strlen(OPT_CHARGE_ANIM_LEVEL_NUM));
		if (level_conf->num <= 0) {
			return false;
		}
	} else {
	...
	if (buf) {
		level_conf->delay =
		    atoi(buf + strlen(OPT_CHARGE_ANIM_DELAY));
	}
		...
		if (!memcmp(arg, OPT_CHARGE_ANIM_LEVEL_CONF,
			...
			only_current_level =
			    !memcmp(arg + strlen(OPT_CHARGE_ANIM_LOOP_CUR),
				    "true", 4);
			LOGD("Found only_current_level:%d",
			     only_current_level);
		} else
		    if (!memcmp
			(arg, OPT_CHARGE_ANIM_LEVELS,
	...
	if (dump) {
		...
		for (i = 0; i < level_conf_num; i++) {
			printf("\tmax=%d, delay=%d, num=%d, prefix=%s\n",
			       level_confs[i].max_level,
			       level_confs[i].delay, level_confs[i].num,
			       level_confs[i].prefix);
		}
	}
...
static resource_content *get_image_content()
{
	...
	if (!conf->images) {
		...
		memset(conf->images, 0,
		       conf->num * sizeof(resource_content));
	}
	if (!conf->images) {
