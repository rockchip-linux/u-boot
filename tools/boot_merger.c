...
uint32_t CRC_32(uint8_t * pData, uint32_t ulSize)
{
	...
	for (i = 0; i < ulSize; i++) {
		nAccum =
		    (nAccum << 8) ^ gTable_Crc32[(nAccum >> 24) ^
						 (*pData++)];
	}
	return nAccum;
}
...
void P_RC4(uint8_t * buf, uint32_t len)
{
	...
	uint32_t i, j, t, x;
	uint8_t key[16] =
	    { 124, 78, 3, 4, 85, 5, 9, 7, 45, 44, 123, 56, 23, 13, 23,
      17 };
...
static bool parse471(FILE * file)
{
	...
	if (gOpts.code471Num < 0)
		return false;
	gOpts.code471Path =
	    (line_t *) malloc(sizeof(line_t) * gOpts.code471Num);
...
static bool parse472(FILE * file)
{
	...
	if (gOpts.code472Num < 0)
		return false;
	gOpts.code472Path =
	    (line_t *) malloc(sizeof(line_t) * gOpts.code472Num);
...
static bool parseLoader(FILE * file)
{
	...
	if (fscanf(file, OPT_NUM "=%d", &gOpts.loaderNum) != 1) {
		fseek(file, pos, SEEK_SET);
		if (fscanf(file, OPT_LOADER_NUM "=%d", &gOpts.loaderNum) !=
		    1) {
			return false;
		}
	}
	...
	if (gOpts.loaderNum < 0)
		return false;
	gOpts.loader =
	    (name_entry *) malloc(sizeof(name_entry) * gOpts.loaderNum);
...
static bool parseOpts()
{
	...
	char *configPath =
	    (gConfigPath == NULL) ? DEF_CONFIG_FILE : gConfigPath;
	FILE *file;
...
static bool writeFile(FILE * outFile, const char *path, bool fix)
{
	...
	if (fix) {
		...
		while (1) {
			P_RC4(buf,
			      fixSize <
			      SMALL_PACKET ? fixSize : SMALL_PACKET);
			buf += SMALL_PACKET;
...
static bool mergeBoot()
{
	...
	for (i = 0; i < gOpts.code471Num; i++) {
		if (!saveEntry
		    (outFile, (char *)gOpts.code471Path[i], ENTRY_471,
	...
	for (i = 0; i < gOpts.code472Num; i++) {
		if (!saveEntry
		    (outFile, (char *)gOpts.code472Path[i], ENTRY_472,
	...
	for (i = 0; i < gOpts.loaderNum; i++) {
		if (!saveEntry
		    (outFile, gOpts.loader[i].path, ENTRY_LOADER, 0,
	...
	for (i = 0; i < gOpts.code471Num; i++) {
		if (!writeFile
		    (outFile, (char *)gOpts.code471Path[i], false))
			goto end;
	}
	LOGD("write code 472\n");
	for (i = 0; i < gOpts.code472Num; i++) {
		if (!writeFile
		    (outFile, (char *)gOpts.code472Path[i], false))
			goto end;
	}
	LOGD("write loader\n");
	for (i = 0; i < gOpts.loaderNum; i++) {
			...
			FILE * inFile)
{
	...
	if (entry->type == ENTRY_LOADER) {
		...
		if (size % SMALL_PACKET) {
			P_RC4(gBuf + i * SMALL_PACKET,
			      size - SMALL_PACKET * 512);
		}
	} else {
		P_RC4(gBuf, size);
	}
...
static bool unpackBoot(char *path)
{
	...
	entrys =
	    (rk_boot_entry *) malloc(sizeof(rk_boot_entry) * entryNum);
...
static void printHelp()
{
	...
	printf
	    ("Merge or unpack Rockchip's loader (Default action is to merge.)\n");
	printf("Options:\n");
	printf("\t" OPT_MERGE
	       "\t\t\tMerge loader with specified config.\n");
	printf("\t" OPT_UNPACK
	       "\t\tUnpack specified loader to current dir.\n");
	printf("\t" OPT_VERBOSE
	       "\t\tDisplay more runtime informations.\n");
...
int main(int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		if (!strcmp(OPT_VERBOSE, argv[i])) {
			...
			if (optPath) {
				fprintf(stderr,
					"only need one path arg, but we have:\n%s\n%s.\n",
					optPath, argv[i]);
				printHelp();
				return -1;
			}
