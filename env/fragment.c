...
{
	if (env_dev == ((desc->uclass_id << 8) | desc->devnum))
		return;
	...
	env_dev = (desc->uclass_id << 8) | desc->devnum;
}
...
static __maybe_unused env_t *envf_read(struct blk_desc *desc)
{
	...
	if (desc->uclass_id == UCLASS_MMC && desc->devnum == 1 &&
...
U_BOOT_ENV_LOCATION(envf) = {
	.location	= ENVL_FRAGMENT,
	.init		= envf_init,
	.load		= envf_load,
