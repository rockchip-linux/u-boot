...
{
	uint32 mux_con = readl(&grf->con_iomux);
...
static void pinctrl_rk322x_i2c_config(struct rk322x_grf *grf, int i2c_id)
{
	...
	case PERIPH_ID_I2C3:
		rk_clrsetreg(&grf->gpio0a_iomux,
			     ...
			     GPIO0A7_I2C1_SDA << GPIO0A7_SHIFT |
			     GPIO0A6_I2C1_SCL << GPIO0A6_SHIFT);
...
static void pinctrl_rk322x_uart_config(struct rk322x_grf *grf, int uart_id)
{
	...
	case PERIPH_ID_UART0:
		rk_clrsetreg(&grf->gpio0c_iomux,
			     GPIO0C1_MASK,
			     GPIO0C1_UART0_RSTN << GPIO0C1_SHIFT);
		rk_clrsetreg(&grf->gpio2d_iomux,
			     GPIO2D5_MASK | GPIO2D3_MASK | GPIO2D2_MASK,
			     GPIO2D5_UART0_CTSN << GPIO2D5_SHIFT |
	...
	case PERIPH_ID_UART1:
		if (mux_con & CON_IOMUX_UART1SEL_MASK)
			rk_clrsetreg(&grf->gpio3b_iomux,
				     GPIO3B5_MASK | GPIO3B6_MASK,
				     GPIO3B5_UART11_SIN << GPIO3B5_SHIFT |
				     GPIO3B6_UART11_SOUT << GPIO3B6_SHIFT);
		else
...
static void pinctrl_rk322x_sdmmc_config(struct rk322x_grf *grf, int mmc_id)
{
	...
	case PERIPH_ID_SDCARD:
		rk_clrsetreg(&grf->gpio1b_iomux,
			     ...
			     GPIO1B6_SDMMC_CMD << GPIO1B6_SHIFT |
			     GPIO1B7_SDMMC_RSTNOUT << GPIO1B6_SHIFT);
