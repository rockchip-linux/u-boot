...
int32 UARTInit(eUART_ch_t uartCh, uint32 baudRate)
{
    ...
    if(uartCh == UART_CH0)
    {
        ...
        pAPBRegStart = (pAPB_REG)REG_FILE_BASE_ADDR;
    	pAPBRegStart->IOMUX_A_CON = (pAPBRegStart->IOMUX_A_CON & 0xffff0fff) | 1<<12 | 1<<14;
    ...
    else
    {
        ...
        pAPBRegStart = (pAPB_REG)REG_FILE_BASE_ADDR;
    	pAPBRegStart->IOMUX_A_CON = (pAPBRegStart->IOMUX_A_CON & 0xf0ffffff) | 1<<24 | 1<<26;
    }
    UARTRest(puartreg);  
...
int fputcs(int ch, void *stream)
{
   if (ch == '\n')
        UARTWriteByte(CONFIG_UART_NUM, '\r');
   UARTWriteByte(CONFIG_UART_NUM, ch);
