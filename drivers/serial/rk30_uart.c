...
void rk29_serial_putc(const char c)
...
void rk29_serial_puts(const char *s)
...
int rk29_serial_init(void)
{
    return(UARTInit(CONFIG_UART_NUM,115200));
}

void rk29_serial_setbrg(void)
{
    UARTSetBaudRate(CONFIG_UART_NUM,115200);
}

int rk29_serial_getc(void)
{
    char a;
