						    ...
						    int page)
{
	...
	writel(index, rknand->regs + NANDC_REG_V6_SPARE0);
	...
	for (step = 1; step <= ecc->steps; step++) {
		...
		if (step < ecc->steps) {
			...
			if (step & 1)
				writel(((u32 *)oob)[0], rknand->regs + NANDC_REG_V6_SPARE1);
			else
				writel(((u32 *)oob)[0], rknand->regs + NANDC_REG_V6_SPARE0);
		}
					  ...
					  struct nand_ecc_ctrl *ecc)
{
	...
	if (ofnode_read_bool(rknand->node, "rockchip,protect-bootrom-blocks"))
...
static int rockchip_nand_chip_init(ofnode node, struct rk_nand *rknand, int devnum)
{
	struct nand_chip *chip;
	struct mtd_info *mtd;
	int ret;
	...
	rknand->node = node;
	ret = ofnode_read_u32(node, "reg", &rknand->banks[devnum]);
	if (ret)
		return -EINVAL;
...
static int rockchip_nand_chips_init(struct rk_nand *rknand)
{
	ofnode child;
	int ret, i = 0;

	dev_for_each_subnode(child, rknand->dev) {
		ret = rockchip_nand_chip_init(child, rknand, i++);
		if (ret)
			return ret;
	}
...
static int rockchip_nandc_probe(struct udevice *dev)
{
	...
	ret = rockchip_nand_chips_init(rknand);
	if (ret)
...
void board_nand_init(void)
{
	...
	node = fdtdec_next_compatible(blob, 0, ROCKCHIP_NANDC);
