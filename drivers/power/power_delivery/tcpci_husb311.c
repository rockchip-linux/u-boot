...
static int husb311_read16(struct husb311_chip *chip, unsigned int reg)
{
	...
	if (ret < 0) {
		printf("%s: Can't read reg 0x%02x, ret=%d!\n",
		       __func__, reg, ret);
		return ret;
	}
...
static int husb311_write8(struct husb311_chip *chip, unsigned int reg, u8 val)
{
	...
	if (ret)
		printf("%s: Can't write 0x%02x to reg 0x%02x, ret=%d!\n",
...
static int husb311_write16(struct husb311_chip *chip, unsigned int reg, u16 val)
{
	...
	if (ret)
		printf("%s: Can't write reg 0x%02x, len=%d, ret=%d!\n",
		       __func__, reg, 2, ret);
...
static int husb311_check_revision(struct husb311_chip *chip)
{
	...
	if (ret < 0) {
		printf("%s: fail to read Vendor ID ret=%d!\n", __func__, ret);
		return ret;
	}

	if (ret != HUSB311_VID) {
		printf("%s: VID 0x%04x is not correct!\n", __func__, ret);
		return -ENODEV;
	}

	ret = husb311_read16(chip, TCPC_PRODUCT_ID);
	if (ret < 0) {
		printf("%s: fail to read Product ID ret=%d!\n", __func__, ret);
		return ret;
	}

	if (ret != HUSB311_PID) {
		printf("%s: PID 0x%04x is not correct!\n", __func__, ret);
		return -ENODEV;
	}
...
static int husb311_probe(struct udevice *dev)
{
	...
	if (ret < 0) {
		printf("%s: check VID/PID fail!\n", __func__);
		return ret;
	}

	ret = husb311_sw_reset(chip);
	if (ret) {
		printf("%s: fail to do soft reset, ret=%d!\n", __func__, ret);
		return ret;
	}
...
U_BOOT_DRIVER(husb311) = {
	...
	.priv_auto = sizeof(struct husb311_chip),
};
