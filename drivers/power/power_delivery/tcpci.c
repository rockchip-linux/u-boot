...
#define tcpc_presenting_cc1_rd(reg) ({ \
	...
	!(TCPC_ROLE_CTRL_DRP & __reg) && \
	((__reg & (TCPC_ROLE_CTRL_CC1_MASK << TCPC_ROLE_CTRL_CC1_SHIFT)) == \
...
#define tcpc_presenting_cc2_rd(reg) ({ \
	typeof(reg) __reg = (reg); \
	!(TCPC_ROLE_CTRL_DRP & __reg) && \
	((__reg & (TCPC_ROLE_CTRL_CC2_MASK << TCPC_ROLE_CTRL_CC2_SHIFT)) == \
...
static int tcpci_read16(struct tcpci *tcpci, unsigned int reg, u16 *val)
{
	...
	if (ret) {
		printf("%s: fail to read reg 0x%02x, ret=%d!\n",
		       __func__, reg, ret);
		return ret;
	}
			    ...
			    u8 *data, u8 length)
{
	...
	if (ret)
		printf("%s: fail to block read reg 0x%02x, len=%d, ret=%d!\n",
...
static int tcpci_write16(struct tcpci *tcpci, unsigned int reg, u16 val)
{
	...
	if (ret)
		printf("%s: fail to write reg 0x%02x, ret=%d!\n",
		       __func__, reg, ret);
			     ...
			     u8 *data, u8 length)
{
	...
	if (ret)
		printf("%s: fail to block write reg 0x%02x, len=%d, ret=%d!\n",
		       __func__, reg, length, ret);
				      ...
				      bool attached, bool pd_capable)
{
	 ...
	 * do not disable 24M oscillator for BMC communication. Otherwise,
...
struct tcpci *tcpci_register_port(struct udevice *dev, struct tcpci_data *data)
{
	...
	if (err) {
		debug("%s: fail to get int GPIO: err=%d\n", __func__, err);
	...
	if (IS_ERR(tcpci->port)) {
		printf("%s: failed to do tcpm port init!\n", __func__);
