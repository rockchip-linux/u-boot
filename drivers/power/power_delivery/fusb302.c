			     ...
			     u8 address, u8 data)
{
	...
	if (ret)
		printf("%s: fail to write 0x%02x to addr 0x%02x, ret=%d\n",
				   ...
				   u8 length, const u8 *data)
{
	...
	if (ret)
		printf("%s: fail to block write addr 0x%02x, len=%d, ret=%d\n",
			    ...
			    u8 address, u8 *data)
{
	...
	if (ret)
		printf("%s: fail to read addr 0x%02x, ret=%d\n",
				  ...
				  u8 length, u8 *data)
{
	...
	if (ret)
		printf("%s: fail to block read addr 0x%02x, len=%d, ret=%d\n",
		       __func__, address, length, ret);
	return ret;
}
...
static int fusb302_sw_reset(struct fusb302_chip *chip)
{
	...
	if (ret)
		printf("Can't do SW reset FUSB302 ret=(%d)!\n", ret);
	else
...
static int fusb302_tcpm_init(struct tcpc_dev *dev)
{
...
static int fusb302_tcpm_get_vbus(struct tcpc_dev *dev)
{
	struct fusb302_chip *chip = container_of(dev, struct fusb302_chip,
						 tcpc_dev);
				...
				enum toggling_mode mode)
{
	...
	if (mode == TOGGLING_MODE_OFF) {
		...
		if (chip->vconn_on)
			printf("Vconn is on during toggle start, must be off!\n");
...
static int fusb302_tcpm_set_cc(struct tcpc_dev *dev, enum typec_cc_status cc)
{
	struct fusb302_chip *chip = container_of(dev, struct fusb302_chip,
						 tcpc_dev);
	...
	if (ret) {
		printf("%s: Can't set toggling mode (%d)!\n", __func__, ret);
		goto done;
	}
	...
	if (ret) {
		printf("%s: Can't set pull-up/-down (%d)!\n", __func__, ret);
		goto done;
	}
	/* reset the cc status */
	chip->cc1 = TYPEC_CC_OPEN;
	...
	if (ret) {
		printf("%s: Can't set SRC current %s (%d)!\n",
	...
	case TYPEC_CC_RP_3_0:
		...
		if (ret) {
			printf("%s: Can't set SRC measure value (%d)!\n",
			       __func__, ret);
			goto done;
		}
		ret = fusb302_i2c_mask_write(chip, FUSB_REG_MASK,
					     ...
					     FUSB_REG_MASK_BC_LVL);
		if (ret) {
			printf("%s: Can't set SRC interrupt (%d)!\n",
			       __func__, ret);
			goto done;
		}
...
static int fusb302_tcpm_get_cc(struct tcpc_dev *dev, enum typec_cc_status *cc1,
			       enum typec_cc_status *cc2)
{
	struct fusb302_chip *chip = container_of(dev, struct fusb302_chip,
						 tcpc_dev);
...
static int fusb302_tcpm_set_polarity(struct tcpc_dev *dev,
				     enum typec_cc_polarity polarity)
{
	return 0;
}

static int fusb302_tcpm_set_vconn(struct tcpc_dev *dev, bool on)
{
	struct fusb302_chip *chip = container_of(dev, struct fusb302_chip,
						 tcpc_dev);
	int ret = 0;
...
static int fusb302_tcpm_set_vbus(struct tcpc_dev *dev, bool on, bool charge)
{
	return 0;
}
...
static int fusb302_tcpm_set_pd_rx(struct tcpc_dev *dev, bool on)
{
	struct fusb302_chip *chip = container_of(dev, struct fusb302_chip,
						 tcpc_dev);
	int ret = 0;
	...
	if (ret) {
		printf("%s: Can't  flush pd rx buffer (%d)!\n", __func__, ret);
		goto done;
	}
	ret = fusb302_pd_tx_flush(chip);
	if (ret) {
		printf("%s: Can't flush pd tx buffer (%d)!\n", __func__, ret);
		goto done;
	}
	ret = fusb302_pd_set_auto_goodcrc(chip, on);
	if (ret) {
		printf("%s: Can't turn %s auto GCRC (%d)!\n",
		       __func__, on ? "on" : "off", ret);
		goto done;
	}
	ret = fusb302_pd_set_interrupts(chip, on);
	if (ret) {
		printf("%s: Can't turn %s pd interrupts (%d)!\n",
		       __func__, on ? "on" : "off", ret);
		goto done;
	}
	debug("%s: PD := %s\n", __func__, on ? "on" : "off");
...
static int fusb302_tcpm_set_roles(struct tcpc_dev *dev, bool attached,
				  enum typec_role pwr,
...
static int fusb302_tcpm_start_toggling(struct tcpc_dev *dev,
...
static int fusb302_tcpm_pd_transmit(struct tcpc_dev *dev,
				    ...
				    unsigned int negotiated_rev)
{
	...
	case TCPC_TX_SOP:
		...
		if (ret)
			printf("%s: Can't update retry count (%d)!\n",
			       __func__, ret);

		ret = fusb302_pd_send_message(chip, msg);
		if (ret)
			printf("%s: Can't send PD message (%d)!\n",
			       __func__, ret);
		break;
	case TCPC_TX_HARD_RESET:
		ret = fusb302_pd_send_hardreset(chip);
		if (ret)
			printf("%s: Can't send hardreset (%d)!\n",
			       __func__, ret);
		break;
	default:
...
static void fusb302_init_tcpc_dev(struct tcpc_dev *fusb302_tcpc_dev)
{
	fusb302_tcpc_dev->init = fusb302_tcpm_init;
	fusb302_tcpc_dev->get_vbus = fusb302_tcpm_get_vbus;
	fusb302_tcpc_dev->set_cc = fusb302_tcpm_set_cc;
	fusb302_tcpc_dev->get_cc = fusb302_tcpm_get_cc;
	fusb302_tcpc_dev->set_polarity = fusb302_tcpm_set_polarity;
	fusb302_tcpc_dev->set_vconn = fusb302_tcpm_set_vconn;
	fusb302_tcpc_dev->set_vbus = fusb302_tcpm_set_vbus;
	fusb302_tcpc_dev->set_pd_rx = fusb302_tcpm_set_pd_rx;
	fusb302_tcpc_dev->set_roles = fusb302_tcpm_set_roles;
	fusb302_tcpc_dev->start_toggling = fusb302_tcpm_start_toggling;
	fusb302_tcpc_dev->pd_transmit = fusb302_tcpm_pd_transmit;
				      ...
				      u8 togdone_result)
{
	...
	if (ret) {
		printf("Can't set CC polarity %s, ret = %d!\n",
	...
	ret = fusb302_set_toggling(chip, TOGGLING_MODE_OFF);
	if (ret) {
		printf("Can't set toggling mode off, ret=%d!\n", ret);
		return ret;
	}
	/* unmask bc_lvl interrupt */
	ret = fusb302_i2c_clear_bits(chip, FUSB_REG_MASK, FUSB_REG_MASK_BC_LVL);
	if (ret) {
		printf("Can't unmask bc_lcl interrupt, ret=%d!\n", ret);
		return ret;
	}
	chip->intr_bc_lvl = true;
				     ...
				     enum typec_cc_status *cc)
{
	...
	debug("Get SRC CC status switches: 0x%0x", status0);
	...
	debug("Get SRC CC status rd_mda status0: 0x%0x", status0);
	...
	debug("Get SRC CC status ra_mda status0: 0x%0x", status0);
	if (status0 & FUSB_REG_STATUS0_COMP)
		*cc = TYPEC_CC_RD;
	else
		*cc = TYPEC_CC_RA;
				      ...
				      u8 togdone_result)
{
	...
	if (cc1 == TYPEC_CC_RD &&
		...
		printf("Unexpected status CC1=%s, CC2=%s, restart toggling!\n",
		       typec_cc_status_name[cc1],
		       typec_cc_status_name[cc2]);
	...
	ret = fusb302_i2c_clear_bits(chip, FUSB_REG_MASK,
				     FUSB_REG_MASK_COMP_CHNG);
	if (ret) {
		printf("Can't unmask comp_chng interrupt, ret=%d!\n", ret);
		return ret;
	}
	chip->intr_comp_chng = true;
	debug("Detected CC1=%s, CC2=%s\n",
...
static void fusb302_interrupt_handle(struct fusb302_chip *chip)
{
	...
	if (interrupta & FUSB_REG_INTERRUPTA_TX_SUCCESS) {
		...
		if (ret) {
			printf("Can't read in PD message, ret=%d!\n", ret);
			return;
		}
	}
