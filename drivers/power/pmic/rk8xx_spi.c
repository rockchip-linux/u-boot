...
#endif
#if CONFIG_IS_ENABLED(IRQ)
static int rk8xx_spi_of_to_plat(struct udevice *dev)
{
...
#else
static inline int rk8xx_spi_of_to_plat(struct udevice *dev)
{
	struct rk8xx_priv *rk8xx = dev_get_priv(dev);
...
static int rk8xx_spi_probe(struct udevice *dev)
{
	...
	slave = spi_setup_slave(spi->seq_, plat->cs[0], plat->max_hz,
				plat->mode);
	if (!slave)
		return -ENODEV;
	priv->slave = slave;
...
U_BOOT_DRIVER(pmic_rk8xx_spi) = {
	...
	.bind = rk8xx_spi_bind,
#endif
	.of_to_plat = rk8xx_spi_of_to_plat,
