...
/* Give a message warning when CONFIG_IRQ is not enabled */
	...
	while (1) {
		...
		if (charging <= 0) {
			...
			if (ret != 0)
				printf("Ebook display reset logo failed\n");
...
show_images:
		...
		if (key_state == BUTTON_ON) {
			...
			printf("screen %s\n", screen_on ? "on" : "off");
		} else if (key_state == BUTTON_ON_HOLD) {
...
static int charge_animation_probe(struct udevice *dev)
{
	...
	ret = button_get_by_code(KEY_POWER, &priv->pwrkey);
	if (ret) {
		printf("Failed to get pwrkey: ret=%d\n", ret);
		return ret;
	}
...
U_BOOT_DRIVER(charge_animation) = {
	...
	.of_to_plat = charge_animation_of_to_plat,
	.plat_auto = sizeof(struct charge_animation_pdata),
	.priv_auto = sizeof(struct charge_animation_priv),
};
