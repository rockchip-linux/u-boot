...
struct crypto_hash_cache *crypto_hash_cache_alloc(crypto_hash_calc direct_calc,
						  void *user_data, u32 data_align, u32 len_align)
{
...
int crypto_hash_update_with_cache(struct crypto_hash_cache *hash_cache,
				  const u8 *data, u32 data_len, bool is_last)
{
	crypto_hash_calc direct_calc = hash_cache->direct_calc;
	...
	if (direct_data_len) {
		...
		if (ret)
			goto error;
		hash_cache->calced_len += direct_data_len;
	}
	...
	if (cache_data_len) {
		...
		if (ret)
			goto error;
		hash_cache->calced_len += cache_data_len;
	}
