...
static enum color_space_type get_color_space_type(u32 color_encoding, bool is_yuv)
{
					...
					struct rk_pq_csc_ventor *out_dc)
{
	...
	case RK_PQ_CSC_V2_VP_R2Y_Y2Y:
		...
		return;
	default:
		printf("Invalid type of swap_channels: %d\n", mode->swap_channels);
		return;
	}
}
			   ...
			   struct post_csc_convert_mode *convert_mode)
{
	...
	if (!csc_coef) {
		printf("get csc index err:\n");
		printf("pixel_depth=%d, coef_precision=%d\n",
		       convert_mode->pixel_depth, convert_mode->coef_precision);
		printf("input: colorspace=%d, yuv=%d, full_range=%d\n",
		       convert_mode->intput_color_encoding, convert_mode->is_input_yuv,
		       convert_mode->is_input_full_range);
		printf("output: colorspace=%d, yuv=%d, full_range=%d\n",
