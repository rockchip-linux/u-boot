...
serdes_graph_get_remote_port(ofnode node, const char *prop_name)
{
	...
	if (!ofnode_valid(ep_node) || !ofnode_is_enabled(ep_node))
		return NULL;
...
static bool serdes_check_route(struct device_node *np, int depth)
{
	u32 reg;
	ofnode node;
	ofnode ports, port;
...
int serdes_route_bind(const struct udevice_id *match)
{
	...
	struct device_node *port_node, *ep_node, *port_parent_node;
	static bool is_bind;
	...
	ofnode_for_each_subnode(node, route_node) {
		...
		if (is_ports_node) {
			...
			if (!ofnode_valid(np_to_ofnode(conn)) ||
			    !ofnode_is_enabled(np_to_ofnode(conn)))
				continue;
