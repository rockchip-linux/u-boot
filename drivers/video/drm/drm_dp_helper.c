...
static int dp_aux_backlight_set_brightness(struct udevice *dev, int percent)
{
	struct dp_aux_backlight *bl = dev_get_priv(dev);
	int level;
	int ret;
	...
	if (!bl->enabled) {
		pr_warn("%s: Call backlight_enable() before setting brightness\n", bl->aux->name);
		ret = backlight_enable(dev);
		if (ret < 0) {
			pr_err("%s: Failed to enable AUX backlight\n", bl->aux->name);
			return ret;
		}
	}

	if (percent == BACKLIGHT_DEFAULT)
		level = bl->default_level;
	else
		level = bl->info.max * percent / 100;

	ret = drm_edp_backlight_set_level(bl->aux, &bl->info, level);
...
static const struct backlight_ops dp_aux_bl_ops = {
	...
	.set_brightness = dp_aux_backlight_set_brightness,
};
...
U_BOOT_DRIVER(dp_aux_backlight) = {
	...
	.priv_auto = sizeof(struct dp_aux_backlight),
};
...
int drm_panel_dp_aux_backlight(struct rockchip_panel *panel, struct drm_dp_aux *aux)
{
	...
	ret = device_bind_with_driver_data(panel->dev, drv, "dp_aux_backlight", (ulong)aux,
					   dev_ofnode(panel->dev), &panel->backlight);
	if (ret)
		return ret;
