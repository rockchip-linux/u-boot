...
static void rk628_combtxphy_dsi_power_on(struct rk628 *rk628)
{
	...
	ret = rk628_read_poll_timeout(rk628, GRF_DPHY0_STATUS, val,
				       val & DPHY_PHYLOCK, 0, 1000);
	if (ret < 0)
		printf("rk628 phy is not lock\n");
...
void rk628_combtxphy_power_on(struct rk628 *rk628)
{
	...
	case RK628_PHY_MODE_VIDEO_MIPI:
		...
		break;
	case RK628_PHY_MODE_VIDEO_LVDS:
		rk628_i2c_update_bits(rk628, GRF_POST_PROC_CON,
				      SW_TXPHY_REFCLK_SEL_MASK,
				      SW_TXPHY_REFCLK_SEL(1));
		rk628_combtxphy_lvds_power_on(rk628);
		break;
	case RK628_PHY_MODE_VIDEO_GVI:
		rk628_i2c_update_bits(rk628, GRF_POST_PROC_CON,
				      SW_TXPHY_REFCLK_SEL_MASK,
...
void rk628_combtxphy_set_mode(struct rk628 *rk628, enum rk628_phy_mode mode)
{
	struct rk628_combtxphy *combtxphy = &rk628->combtxphy;
	...
	case RK628_PHY_MODE_VIDEO_GVI:
	{
		...
		ref_clk = DIV_ROUND_CLOSEST(ref_clk, 1000); /* khz */
