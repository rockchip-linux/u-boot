...
int rk628_lvds_parse(struct rk628 *rk628, ofnode lvds_np)
{
	const char *string = NULL;
	int ret;

	if (!ofnode_valid(lvds_np))
		return -EINVAL;

	string = ofnode_read_string(lvds_np, "bus-format");
	...
	string = ofnode_read_string(lvds_np, "link-type");
