...
static int generic_edp_panel_probe(struct udevice *dev, struct rockchip_panel *base)
{
	 ...
	 */
	ofnode_read_u32(dev->node, "hpd-reliable-delay-ms", &reliable_ms);
	desc->delay.hpd_reliable = reliable_ms;
	ofnode_read_u32(dev->node, "hpd-absent-delay-ms", &absent_ms);
...
U_BOOT_DRIVER(rockchip_edp_panel) = {
	...
	.priv_auto_alloc_size = sizeof(struct panel_edp),
};
