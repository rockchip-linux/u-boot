...
__maybe_unused static int rockchip_usb2phy_reset(struct rockchip_usb2phy *rphy)
{
	int ret;
...
static int rockchip_usb2phy_power_on(struct phy *phy)
{
	...
	if (vbus) {
		...
		if (ret && ret != -EACCES) {
			pr_err("%s: Failed to set VBus supply, ret=%d\n", __func__, ret);
			return ret;
		}
	}
...
static int rockchip_usb2phy_probe(struct udevice *dev)
{
	...
	if (IS_ERR(rphy->phy_base)) {
		dev_info(dev, "get the base address of usb phy failed\n");
	}
...
#endif
#ifdef CONFIG_ROCKCHIP_RK3538
	{ .compatible = "rockchip,rk3538-usb2phy", .data = (ulong)&rk3538_phy_cfgs },
#endif
#ifdef CONFIG_ROCKCHIP_RK3562
