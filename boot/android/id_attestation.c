...
uint32_t optee_write_keybox(u8 *filename, uint32_t filename_size,
...
uint32_t read_id_attestation_digest(const char *ids_digest_file, uint32_t *ids_digest_len)
{
	...
	uint32_t ret = optee_read_keybox((u8 *)ids_digest_file, strlen(ids_digest_file),
					 (u8 *)ids_digest, len);
	if (ret != TEE_SUCCESS)
		*ids_digest_len = 0;
	else
...
{
	uint32_t ret = optee_write_keybox((u8 *)ids_file, strlen(ids_file),
					     (u8 *)ids, ids_len);
...
atap_result write_id_attestation_to_secure_storage(u8* received_data, uint32_t len)
{
	...
	uint32_t ret = optee_read_keybox((u8 *)ids_file, strlen(ids_file),
					 (u8*)&ids_read, sizeof(AttestationIds));
