 ...
 * Copyright (C) 2017-2023 Rockchip Electronics Co., Ltd
 *
...
u32 android_bcb_msg_sector_offset(void)
{
	 ...
	 * are using, then we could able to use plat_boot_mode()
		      ...
		      ulong blkstart, void *ram_base)
{
	...
	struct disk_partition part_init_boot;
...
static ulong android_image_get_comp_addr(struct andr_img_hdr *hdr, int comp)
{
	...
	if (comp == IH_COMP_NONE) {
		if (comp_addr) {
			...
			load_addr = CFG_SYS_SDRAM_BASE + 0x8000;
...
long android_image_load(struct blk_desc *dev_desc,
	...
	hdr = populate_andr_img_hdr(dev_desc, (struct disk_partition *)part_info);
	if (!hdr) {
...
extract_vendor_boot_image_v34_header(struct blk_desc *dev_desc,
				     const struct disk_partition *part_vendor_boot)
{
