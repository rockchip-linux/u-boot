...
static int sysmem_add(phys_addr_t base, phys_size_t size)
{
	...
	ret = lmb_lg_add(&sysmem->lmb, base, size);
	if (ret < 0)
				     ...
				     ulong align)
{
	...
	if (base != SYSMEM_ALLOC_ANYWHERE && base < CFG_SYS_SDRAM_BASE)
		return (void *)base;
	if ((base + size >= CFG_SYS_SDRAM_BASE + SDRAM_MAX_SIZE) &&
	    (base + size <= SZ_4G))
	   	return (void *)base;
	...
	paddr = lmb_lg_alloc_base(&sysmem->lmb, alloc_size, align, alloc_base);
	if (paddr) {
		if ((paddr == base) || (base == SYSMEM_ALLOC_ANYWHERE)) {
			...
			if (lmb_lg_free(&sysmem->lmb, paddr, alloc_size) < 0)
...
ulong sysmem_alloc_temporary_mem(phys_size_t size)
{
	...
	base = (gd->start_addr_sp - CONFIG_STACK_SIZE - 0x2000) - size;

	/* LMB is align down alloc mechanism */
	alloc_base = base + size;
	paddr = __lmb_lg_alloc_base(&sysmem->lmb, size, SZ_1K, alloc_base);
	if (paddr) {
		/* If free failed, return false */
		ret = lmb_lg_free(&sysmem->lmb, paddr, size);
		if (ret < 0) {
...
int sysmem_free(phys_addr_t base)
{
	...
	ret = lmb_lg_free(&sysmem->lmb, mem->base, mem->size);
	if (ret >= 0) {
...
int sysmem_init(void)
{
	...
	mem_start = gd->start_addr_sp - CONFIG_STACK_SIZE;
	mem_size = CONFIG_STACK_SIZE;
...
static int do_sysmem_dump(struct cmd_tbl *cmdtp, int flag,
...
static int do_sysmem_search(struct cmd_tbl *cmdtp, int flag,
			    int argc, char *const argv[])
{
	ulong addr, size;
