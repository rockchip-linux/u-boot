...
{   sha256_ctx  cx[1];
    ...
    sha256_begin(cx); sha256_hash(cx, data, len); sha256_end(cx, hval);
}
...
static void sha_end(sha512_ctx ctx[1], unsigned char hval[], const unsigned int hlen)
...
{   sha384_ctx  cx[1];
    ...
    sha384_begin(cx); sha384_hash(cx, data, len); sha384_end(cx, hval);
}
...
{   sha512_ctx  cx[1];
    ...
    sha512_begin(cx); sha512_hash(cx, data, len); sha512_end(cx, hval);
}
...
void sha2_hash(sha2_ctx ctx[1], const unsigned char data[], unsigned long len)
{
    switch(ctx->sha2_len)
    {
        case 32: sha256_hash(CTX_256(ctx), data, len); return;
        case 48: sha384_hash(CTX_384(ctx), data, len); return;
        case 64: sha512_hash(CTX_512(ctx), data, len); return;
    }
}
...
void sha2_end(sha2_ctx ctx[1], unsigned char hval[])
{
    switch(ctx->sha2_len)
    {
        case 32: sha256_end(CTX_256(ctx), hval); return;
        case 48: sha_end(CTX_384(ctx), hval, SHA384_DIGEST_SIZE); return;
        case 64: sha_end(CTX_512(ctx), hval, SHA512_DIGEST_SIZE); return;
    }
}
