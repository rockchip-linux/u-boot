...
static int bidram_add(phys_addr_t base, phys_size_t size)
{
	...
	ret = lmb_lg_add(&bidram->lmb, base, size);
	if (ret < 0)
...
{
	struct bidram *bidram = &plat_bidram;
	struct lmb_lg *lmb = &plat_bidram.lmb;
	struct lmb_lg_property *mem_rgn = lmb->memory.region;
	struct lmb_lg_property *res_rgn = lmb->reserved.region;
			       ...
			       phys_addr_t base, phys_size_t size)
{
	...
	ret = lmb_lg_reserve(&bidram->lmb, base, size);
	if (ret >= 0) {
...
phys_size_t bidram_get_ram_size(void)
{
	...
	phys_size_t ram_addr_end = CFG_SYS_SDRAM_BASE;
	...
	ret = bidram_add(CFG_SYS_SDRAM_BASE,
			 ram_addr_end - CFG_SYS_SDRAM_BASE);
	if (ret) {
	...
	BIDRAM_D("DRAM size: 0x%08lx\n",
		 (ulong)ram_addr_end - CFG_SYS_SDRAM_BASE);

#ifdef DEBUG
	bidram_dump();
#endif

	return (ram_addr_end - CFG_SYS_SDRAM_BASE);
}
...
static int do_bidram_dump(struct cmd_tbl *cmdtp, int flag,
