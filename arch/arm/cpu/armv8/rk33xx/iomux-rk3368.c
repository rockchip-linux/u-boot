...
static void rk_lcdc_iomux_config(int lcd_id)
{
	switch (lcd_id) {
		case RK_LCDC0_IOMUX:
			pmugrf_writel((0xff<<(8 + 16)) | (0x55 << 8), PMU_GRF_GPIO0D_IOMUX);  //lcdc iomux
			break;
		default :
...
static void rk_emmc_iomux_config(int emmc_id)
{
	switch (emmc_id) {
		case RK_EMMC_IOMUX:
			grf_writel((0xfff0<<16) | 0xaaa0, GRF_GPIO1C_IOMUX); // emmc data0-5
			grf_writel((3<<18) | (3<<16) | (2<<2) | (2<<0), GRF_GPIO1D_IOMUX); // emmc data6-7
			grf_writel((3<<22) | (3<<20) | (2<<6) | (2<<4), GRF_GPIO1D_IOMUX); // emmc pwren, emmc cmd
			grf_writel((3<<24) | (3<<22) | (2<<8) | (2<<6), GRF_GPIO2A_IOMUX); // emmc rstn out, emmc clkout
			break;
		default:
...
static uint32 grf_gpio2a_iomux = 0, grf_gpio2b_iomux = 0;
