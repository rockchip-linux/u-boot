...
#define _APLL_SET_CLKS(_khz, _nr, _nf, _no, _core_div, _periph_div, _axi_core_div, _axi_div, _ahb_div, _apb_div, _ahb2apb) \
{ \
	.rate	= _khz * KHZ, \
	.nr	= _nr, \
	.nf	= _nf, \
...
static const struct pll_clk_set apll_clks[] = {
	//rate, nr, nf, no,	core_div, peri_div, axi_core_div, axi_div, hclk_div, pclk_div, ahb2apb_div
	_APLL_SET_CLKS(816000, 1, 34, 1,    1, 8, 4, 3, 2, 4, 2),
	_APLL_SET_CLKS(600000, 1, 25, 1,    1, 4, 4, 3, 2, 4, 2),
};
...
static int rkclk_pll_clk_set_rate(enum rk_plls_id pll_id, uint32 mHz, pll_callback_f cb_f)
{
	...
	/* enter rest */
        ...
        g_cruReg->CRU_PLL_CON[pll_id][2] = PLL_CLK_BWADJ_SET(nf >> 1);
