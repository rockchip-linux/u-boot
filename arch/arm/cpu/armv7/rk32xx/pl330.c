...
struct pl330_dmac {
	...
	void 			(*dotask)(unsigned long data);
...
#define msecs_to_loops(t) (t * 1000 / CONFIG_SYS_HZ)

/* Returns Time-Out */
static bool _until_dmac_idle(struct pl330_thread *thrd)
{
	void __iomem *regs = thrd->dmac->pinfo->base;
	u32 loops = msecs_to_loops(5);
	u32 TimeOutBase = get_timer(0);
	u32 TimeOut = 0;
		...
		u8 insn[], bool as_manager)
{
	...
	/* cwz: fixed &insn[2] address not 4-byte aligned data abort */
#if 0
	val = *((u32 *)&insn[2]);
#else
...
int pl330_submit_req(void *ch_id, struct pl330_req *r)
{
	...
	if (r->rqtype != MEMTOMEM && r->peri >= pi->pcfg.num_peri) {
		rk_pl330_dev_info("%s:%d Invalid peripheral(%u)!\n",
	...
	if (!_is_valid(ccr)) {
		ret = -EINVAL;
		rk_pl330_dev_info("%s:%d Invalid CCR(%x)!\n",
...
int pl330_update(const struct pl330_info *pi)
{
	...
	if (val) {
		...
		while (i < pi->pcfg.num_chan) {
			if (val & (1 << i)) {
				rk_pl330_dev_info("Reset Channel-%d\t CS-%x FTC-%x\n",
						i, readl(regs + CS(i)),
						readl(regs + FTC(i)));
	...
	if (pi->pcfg.num_events < 32
		...
		rk_pl330_dev_err("%s:%d Unexpected!\n", __func__, __LINE__);
		ret = 1;
		goto updt_exit;
	}
...
static int dmac_alloc_resources(struct pl330_dmac *pl330)
{
	...
	if (!pl330->mcode_cpu) {
		rk_pl330_dev_err("%s:%d Can't allocate memory!\n",
			__func__, __LINE__);
		return -ENOMEM;
	}
	...
	if (ret) {
		rk_pl330_dev_err("%s:%d Can't to create channels for DMAC!\n",
			__func__, __LINE__);
...
int pl330_add(struct pl330_info *pi)
{
	...
	if ((get_id(pi, PERIPH_ID) & 0xfffff) != PERIPH_ID_VAL
	   || get_id(pi, PCELL_ID) != PCELL_ID_VAL) {
		rk_pl330_dev_err("PERIPH_ID 0x%x, PCELL_ID 0x%x !\n",
	...
	if (pi->pcfg.num_events == 0) {
		rk_pl330_dev_err("%s:%d Can't work without events!\n",
			__func__, __LINE__);
		return -EINVAL;
	}
	...
	if (ret) {
		rk_pl330_dev_err("Unable to create channels for DMAC\n");
		free(pl330);
		return ret;
	}

	pl330->dotask = pl330_dotask;
	pl330->state = INIT;
